...
The Heist is an AR puzzle game for the Snap Specs. It features Solo and Team modes - with networking powered by Snap Cloud/Supabase. This repository contains the release version TypeScript code & Lens Studio project files for reference.

This project originally won 1st place in a Lenslist Spectacles challenge. Recently, we rebuilt the codebase from the ground up. The current version has upgraded visuals, cleaner Spectacles UIKit integration, and a full TypeScript rewrite across all scripts.

## Why open source?

This repo is meant to be a practical reference for:
- Gameplay state structure in Lens Studio
- Reusable interaction components for AR puzzles (Buttons, Wires)
- Snap Cloud room creation and realtime camera streaming from the Specs
...
This is the networking system for co-op.
