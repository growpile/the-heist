...
script.disconnectFromRoom = function () {
    isReady = false;
    isSubscribed = false;
    lastRoomCode = sessionChannelName || lastRoomCode;
    sessionChannelName = "";
...
script.deleteCurrentRoom = async function () {
    var roomCode = sessionChannelName || lastRoomCode;
    if (!client || !roomCode) {
        log("Cannot delete room: client or room code missing");
        return false;
    }
    try {
        var res = await client.from("bomb_defusal_rooms").delete().eq("room_code", roomCode);
        if (res && res.error) {
            log("Delete room failed: " + JSON.stringify(res.error));
            return false;
        }
        log("Deleted room for code " + roomCode);
        return true;
    } catch (e) {
