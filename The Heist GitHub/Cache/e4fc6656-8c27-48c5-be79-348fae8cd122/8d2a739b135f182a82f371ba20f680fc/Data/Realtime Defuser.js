...
async function ensureRoomRegistration() {
    ...
    try {
        ...
        if (hasExisting) {
            ...
            // Generate a new code different from the existing one
            var refreshedCode = await generateUniqueRoomCode(table, existingCode);
            if (!refreshedCode) {
                log("Could not generate new room code to replace existing");
                return existingCode;
            }
            await client.from(table).update({
                room_code: refreshedCode,
    ...
    for (var attempt = 0; attempt < 20; attempt++) {
        ...
        try {
            ...
            if (code === "23505" || code === "PGRST116" || msg.indexOf("duplicate key") !== -1) {
                // Conflict on room_code; try another
                continue;
            }
