...
// @input string channelName = "demo"
// @input string baseMessage = "Spectacles ping"
...
async function setupRealtimeChannel() {
    if (!client) { return; }
    var channelName = script.channelName || "demo";
    realtimeChannel = client.channel("simple-" + channelName, {
...
function sendRandomMessage() {
    if (!isReady || !realtimeChannel) {
        log("Not ready yet; initialize first");
        return;
    }
    ...
    realtimeChannel.send({
        ...
        payload: {
            channel_name: script.channelName || "demo",
            user_id: userId,
            message: message,
            random: randomNumber,
            timestamp: Date.now()
