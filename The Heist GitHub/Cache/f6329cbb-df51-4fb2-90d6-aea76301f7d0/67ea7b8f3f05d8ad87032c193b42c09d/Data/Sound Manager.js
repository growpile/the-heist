...
function stopSfxById(soundId) {
    ...
    var so = null;
    try {
        so = active.getSceneObject ? active.getSceneObject() : null;
    } catch (e) {
        so = null;
    }
    if (so) { so.destroy(); }
...
global.playSfx = function(id, times, volume, soundId)
{
    ...
    if (soundId) {
        ...
        taggedTrack.setOnFinish(function(ac){
            ...
            if (!ac || ac.__manualStop) { return; }
            var so = null;
            try {
                so = ac.getSceneObject ? ac.getSceneObject() : null;
            } catch (e) {
                so = null;
            }
            if (so) { so.destroy(); }
...
global.setBgmVolume = function(volume, fadeDuration)
{
    if (!bgm) { return; }
