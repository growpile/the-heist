...
export class SafeRotationManager extends BaseScriptComponent {
  ...
  private baseEuler: vec3 | null = null
  ...
  resetRotation(): void {
    ...
    if (!this.baseEuler) {
      this.baseEuler = this.getQuatEuler(this.rotateTransform.getLocalRotation())
    }
    this.setCanRotate(false)
    if (this.accumulatedYaw === 0) {
      this.rotateTransform.setLocalRotation(quat.fromEulerVec(this.baseEuler))
      return
    }
  ...
  private getQuatEuler(q: quat): vec3 {
    if (!q) {
      return new vec3(0, 0, 0)
    }
    if ((q as any).toEulerAngles) {
      return (q as any).toEulerAngles() as vec3
    }
    if ((q as any).toEulerVec) {
      return (q as any).toEulerVec() as vec3
    }
    return new vec3(0, 0, 0)
  }
  ...
  private animateYawDelta(deltaRadians: number, durationSec: number): void {
    ...
    const startYaw = this.accumulatedYaw
    const targetYaw = this.accumulatedYaw + deltaRadians
    const targetEuler = new vec3(this.baseEuler.x, this.baseEuler.y + targetYaw, this.baseEuler.z)
    ...
    const tween = LSTween.rawTween(durationMs)
      .onUpdate((t: number) => {
        const yaw = startYaw + (targetYaw - startYaw) * this.smoothStep(t)
        this.rotateTransform?.setLocalRotation(
          quat.fromEulerVec(new vec3(this.baseEuler!.x, this.baseEuler!.y + yaw, this.baseEuler!.z))
        )
      })
      .onComplete(() => {
        this.rotateTransform?.setLocalRotation(quat.fromEulerVec(targetEuler))
        this.accumulatedYaw = targetYaw
  ...
  private animateGroundRotation(deltaRadians: number, durationSec: number): void {
    if (!this.groundMaterial?.mainPass || this.groundMaterial.mainPass.rotation === undefined) {
      return
    }

    const groundStart = this.groundMaterial.mainPass.rotation as number
    ...
    const tween = LSTween.rawTween(durationMs)
      .onUpdate((t: number) => {
        ...
        this.groundMaterial!.mainPass.rotation =
          groundStart + (groundTarget - groundStart) * smoothT
      })
      .onComplete(() => {
        this.resetGroundRotation()
    ...
    this.groundTween = tween
    tween.start()
  }

  private resetGroundRotation(): void {
    if (this.groundMaterial?.mainPass && this.groundMaterial.mainPass.rotation !== undefined) {
      this.groundMaterial.mainPass.rotation = 0
    }
  }
  ...
  private animateMaterialScalar(
    ...
    const tween = LSTween.rawTween(durationMs)
      .onUpdate((t: number) => {
        const value = startValue + (targetValue - startValue) * this.smoothStep(t)
  ...
  private getColliderBox(body: BodyComponent | undefined): AreaBox | null {
    ...
    const size = collider.shape.size
