...
export class MenuController extends BaseScriptComponent {
  ...
  @ui.separator
  @ui.label('<span style="color: #94A3B8;">Post-Game Session</span>')
  ...
  private resetParentChildren(parent: SceneObject | undefined): void {
    if (!parent) {
      return
    }
  ...
  private computeStarCount(solveDurationSec: number, bombTimerSec: number): number {
  ...
  private popInPostGameStars(
    ...
    for (let i = 0; i < visibleStars.length; i++) {
      ...
      global.utils.delay(delaySec, () => {
        ...
        try {
          LSTween.scaleFromToLocal(transform, vec3.zero(), restScale, POSTGAME_STAR_POP_SEC * 1000)
