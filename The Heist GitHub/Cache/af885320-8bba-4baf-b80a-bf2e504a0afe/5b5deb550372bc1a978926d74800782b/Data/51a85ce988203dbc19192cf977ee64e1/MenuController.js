...
let MenuController = (() => {
    ...
    var MenuController = _classThis = class extends _classSuper {
        ...
        /** Scale menu root from zero and run tab pop-in after gameplay. */
        ...
        /** Scale menu away and hide step views before surface placement. */
        ...
        /** Win/fail panel after safe scales away — stars, time, menu button. */
         ...
         */
        /** Swap main-menu chrome vs settings child under mainMenuView.root. */
        setSettingsOverlayActive(active) {
            const mainRoot = this.mainMenuView?.root;
            if (!mainRoot || !this.settingsView) {
                return;
            }
        ...
        /** Re-enable roots and main shell before scaling the menu back in. */
        ...
        runGameplayMenuReveal(restoreId, callback) {
            ...
            const menuRootAlreadyVisible = !!this.menuRoot &&
        ...
        onPostGameMenuPressed() {
            this.logMenu("onPostGameMenuPressed postGameActive=" +
                this.postGameActive +
                " hasHandler=" +
        ...
        setTabButtonsInteractable(enabled) {
            // Intentionally no-op: selection locks are enforced in selectTab() via
            // isScreenChangeLocked/tabSwitchLocked without mutating button components.
            void enabled;
        }
        beginTabTransition() {
            this.tabTransitionToken++;
        ...
        updateTabToggles() {
            // Clear all toggles first so UIKit visual state never keeps a stale active tab.
            for (let i = 0; i < this.tabs.length; i++) {
                const button = this.getTabButton(this.tabs[i]);
                if (!button) {
                    continue;
            ...
            // Pulse inactive after clearing toggles so UIButton state fully refreshes.
            for (let i = 0; i < this.tabs.length; i++) {
                const button = this.getTabButton(this.tabs[i]);
                if (!button || typeof button.inactive === "undefined") {
                    continue;
                }
                button.inactive = true;
                button.inactive = false;
            }
            // Then apply the selected tab toggle.
            for (let i = 0; i < this.tabs.length; i++) {
                const button = this.getTabButton(this.tabs[i]);
                if (!button) {
                    continue;
