...
export class MenuController extends BaseScriptComponent {
  ...
  returnFromGameplay(callback?: () => void): void {
    if (!this.tryBeginScreenChange()) {
      // Recovery path: if lock is stale, force a safe menu restore instead of dropping the request.
      print("[MenuController] returnFromGameplay — lock busy, forcing snap recovery")
