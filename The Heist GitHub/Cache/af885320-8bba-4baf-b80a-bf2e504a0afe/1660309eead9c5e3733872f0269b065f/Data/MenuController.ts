...
export class MenuController extends BaseScriptComponent {
  ...
  private transitionToScreen(target: MenuScreenId, callback?: () => void): void {
    if (target === this.currentScreen && !this.isTransitioning) {
      callback?.()
      return
    }
    ...
    const fromElements = this.getScreenElements(from)
    ...
    const popInTarget = () => {
      ...
      this.transitions.prepareElementsForPopIn(toElements)
      this.transitions.popInElements(toElements, finish)
    }
  ...
  private getScreenElements(screen: MenuScreenId): SceneObject[] {
    const view = this.getViewForScreen(screen)
    return this.resolveElements(view)
  }
  ...
  private resolveElements(view: MenuView | undefined): SceneObject[] {
    ...
    return this.collectDirectChildren(view.root)
  }
  ...
  private cacheAllViewElements(): void {
    ...
    this.transitions.cacheRestScales(all)
  }
