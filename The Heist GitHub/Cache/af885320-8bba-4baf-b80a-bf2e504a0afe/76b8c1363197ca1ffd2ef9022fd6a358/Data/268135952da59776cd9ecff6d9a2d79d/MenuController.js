...
let MenuController = (() => {
    ...
    var MenuController = _classThis = class extends _classSuper {
         ...
         */
        popMainMenuAndShowRoom(callback) {
            if (!this.menuRoot) {
                callback?.();
                return;
            }
        ...
        restoreMainMenuFromRoom(callback) {
            const mainMenu = this.getMainMenuShell();
            const showMain = () => {
                if (!mainMenu) {
                    callback?.();
        ...
        animateViewScale(target, toScale, durationMs, onComplete) {
            const durationSec = durationMs / 1000;
            if (global.utils && typeof global.utils.animateScale === "function") {
                global.utils.animateScale(target, true, toScale, durationSec, () => {
                    onComplete?.();
            ...
            try {
                this.viewTween = LSTween_1.LSTween.scaleToLocal(target.getTransform(), toScale, durationMs)
                    .easing(toScale.x > 0 ? Easing_1.default.Back.Out : Easing_1.default.Quadratic.In)
                    .onComplete(() => {
                    this.viewTween = null;
                    onComplete?.();
