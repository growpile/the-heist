...
const POSTGAME_STAR_IMAGE_TARGET_ALPHA = 0.7;
    ...
    var MenuController = _classThis = class extends _classSuper {
        ...
        fadeInStarImage(image) {
            ...
            if (material && global.utils?.animateMaterialProperty) {
                if (this.hasMaterialProperty(material, "mainPass.opacityMultiplier")) {
                    global.utils.animateMaterialProperty(material, "mainPass.opacityMultiplier", POSTGAME_STAR_IMAGE_TARGET_ALPHA, POSTGAME_STAR_IMAGE_FADE_SEC);
                    return;
                }
                if (this.hasMaterialProperty(material, "mainPass.opacity")) {
                    global.utils.animateMaterialProperty(material, "mainPass.opacity", POSTGAME_STAR_IMAGE_TARGET_ALPHA, POSTGAME_STAR_IMAGE_FADE_SEC);
                    return;
                }
                if (this.hasMaterialProperty(material, "mainPass.baseColor.a")) {
                    global.utils.animateMaterialProperty(material, "mainPass.baseColor.a", POSTGAME_STAR_IMAGE_TARGET_ALPHA, POSTGAME_STAR_IMAGE_FADE_SEC);
                    return;
                }
            }
        ...
        setImageOpacity(image, alpha) {
            ...
            if (material.mainPass && material.mainPass.opacityMultiplier !== undefined) {
                material.mainPass.opacityMultiplier = a;
                return;
            }
            if (material.mainPass && material.mainPass.opacity !== undefined) {
                material.mainPass.opacity = a;
                return;
            }
            if (material.mainPass && material.mainPass.baseColor) {
                const c = material.mainPass.baseColor;
                material.mainPass.baseColor = new vec4(c.r, c.g, c.b, a);
            }
        }
        hasMaterialProperty(material, path) {
            const parts = path.split(".");
            let root = material;
            for (let i = 0; i < parts.length; i++) {
                if (!root || root[parts[i]] === undefined) {
                    return false;
                }
                root = root[parts[i]];
            }
