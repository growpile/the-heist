...
export class MenuController extends BaseScriptComponent {
  ...
  @ui.separator
  @ui.label('<span style="color: #60A5FA;">Post-Game Session</span>')
  ...
  private fadeInStarImage(image: Image | null): void {
    ...
    if (material && global.utils?.animateMaterialProperty) {
      if (this.hasMaterialProperty(material, "mainPass.opacityMultiplier")) {
        global.utils.animateMaterialProperty(
          material,
          "mainPass.opacityMultiplier",
          POSTGAME_STAR_IMAGE_TARGET_ALPHA,
          POSTGAME_STAR_IMAGE_FADE_SEC
        )
        return
      }
  ...
  private hasMaterialProperty(material: Material, path: string): boolean {
    const parts = path.split(".")
    let root: any = material as any
