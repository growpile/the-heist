...
export class MenuController extends BaseScriptComponent {
  ...
  /** Disable step views only — menuRoot / Headlock stay untouched. */
  ...
  /** One-shot Headlock reset — does not disable the component. */
  private snapHeadlockToOffset(): void {
    if (!this.menuRoot) {
      return
    }
    ...
    for (const comp of scriptComponents) {
      const snap = (comp as {snapToOffsetPosition?: () => void}).snapToOffsetPosition
      if (snap) {
        snap.call(comp)
        return
      }
