...
function setupWire(i) {
    ...
    if (i < activeWiresCount) {
        ...
        // ensure at least 3 distinct colors per generation
        var palette = Object.values(Colors);
        var colorPool = palette.slice();
        var minUnique = Math.min(3, activeWiresCount);
        if (i < minUnique) {
            knuthShuffle(colorPool);
            wireColorCounts[colorPool[i].name]++;
            var forcedColor = colorPool[i];
            var textComponent = buttonSO.getChild(0).getComponent("Component.Text");
            textComponent.text = "Wire " + (i + 1);
            textComponent.textFill.color = forcedColor.color;
            print("Wire " + (i + 1) + " is active. Color: " + forcedColor.name);
            wires[i] = {color: forcedColor, isEven: ((i + 1) % 2 === 0)};
            return;
        }

        var colorData = palette[randomInt(0, palette.length - 1)];
