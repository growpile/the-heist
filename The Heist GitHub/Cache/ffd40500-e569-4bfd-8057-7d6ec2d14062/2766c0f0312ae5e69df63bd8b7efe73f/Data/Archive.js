...
function createWires(onSuccess) {
    ...
    var minUnique = Math.min(3, activeWiresCount, palette.length);
    var shuffled = palette.slice();
    knuthShuffle(shuffled);
    for (i = 0; i < minUnique; i++) {
        plannedWireColors.push(shuffled[i]);
    }
    for (i = minUnique; i < activeWiresCount; i++) {
        plannedWireColors.push(palette[randomInt(0, palette.length - 1)]);
    }
...
function setupWire(i) {
    ...
    if (i < activeWiresCount) {
        ...
        var colorData = plannedWireColors[i] || Object.values(Colors)[randomInt(0, Object.values(Colors).length - 1)];
...
function createRules() {
    ...
    function cutTwoWires() {
        ...
        while (secondPos === firstPos && activeWiresCount > 1) {
            secondPos = randomInt(1, activeWiresCount);
        }
        return {
            id: "cut-two-wires",
    ...
    function cutWireBasedOnOtherWire() {
        ...
        while (g === m && activeWiresCount > 1) {
            g = randomInt(1, activeWiresCount);
        }
        var color = colors[randomInt(0, colors.length - 1)];
        return {
    ...
    function ifMoreColor1ThanColor2() {
        ...
        while (c2 === c1 && colors.length > 1) {
            c2 = colors[randomInt(0, colors.length - 1)];
        }
        return {
            id: "if-more-color",
    ...
    function makeRuleEntries() {
        ...
        var entries = [];
        var includeUnique = serial.containsWord || randomInt(0, 1) === 1;
        if (includeUnique) {
            var uniqueRule;
            if (serial.containsWord) {
                uniqueRule = cutBasedOnSerialReversedWord();
            } else {
                uniqueRule = uniqueFactories[randomInt(0, uniqueFactories.length - 1)]();
            }
            entries.push({ rule: uniqueRule, isUnique: true });
        }
    ...
    function applyRulesOnce(plannedCutColors) {
        ...
        for (var i = 0; i < currentRuleEntries.length; i++) {
            ...
            if (instruction) {
                ...
                if (instruction.type === InstructionType.Permissive && entry.rule.plannedColor) {
                    plannedCutColors[entry.rule.plannedColor] = true;
                } else if (instruction.type === InstructionType.Permissive && instruction.message.indexOf("Cut") === 0) {
    ...
    puzzleActive = true;
    cutCount = 0;
    print("Generated rules. Solution: " + JSON.stringify(solution));
}
...
function attemptWireCut(wireId) {
    ...
    if (!wireButtons[wireId].enabled) return;
