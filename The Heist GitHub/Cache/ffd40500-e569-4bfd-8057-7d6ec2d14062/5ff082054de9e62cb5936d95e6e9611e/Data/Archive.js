...
function ruleBuilder() {
    ...
    // generate serial number, have a 20% chance of being a reversed word
    var serialNumber = "";
    ...
    if (Math.random() < 0.2) {
        ...
        // generate seruial number that alternates letters and digits
        var letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        var digits = "0123456789";
        for (var i = 0; i < 8; i++) {
            if (i % 2 === 0) {
                serialNumber += letters.charAt(randomInt(0, letters.length - 1));
            } else {
                serialNumber += digits.charAt(randomInt(0, digits.length - 1));
            }
        }
    }
