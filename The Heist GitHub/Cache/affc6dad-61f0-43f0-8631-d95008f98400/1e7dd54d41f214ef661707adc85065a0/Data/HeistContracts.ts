...
export function asGameFlowFacade(comp: ScriptComponent | GameFlowFacade | null | undefined): GameFlowFacade | null {
  ...
  const count = sceneObject.getComponentCount("Component.ScriptComponent")
  for (let i = 0; i < count; i++) {
    const c = sceneObject.getComponentByIndex("Component.ScriptComponent", i) as ScriptComponent
    if (!c) {
      continue
    }
