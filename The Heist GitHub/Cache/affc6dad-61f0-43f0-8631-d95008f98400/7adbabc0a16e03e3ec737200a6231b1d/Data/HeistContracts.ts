...
export function resolveScriptFacade<T>(
  ...
  const comp = sceneObject.getComponent(typeName) as ScriptComponent
  return (comp as unknown as T) ?? null
}
...
export function asGameFlowFacade(comp: ScriptComponent | GameFlowFacade | null | undefined): GameFlowFacade | null {
  ...
  for (let i = 0; i < sceneObject.getComponentsCount(); i++) {
    const c = sceneObject.getComponent("Component.ScriptComponent") as ScriptComponent
    if (c) {
