...
var START_WORLD_POS = new vec3(0, 34, -10);
var END_WORLD_POS = new vec3(0, 70, -10);
...
function cacheBase() {
    ...
    baseWorldPos = targetTransform.getWorldPosition();
...
function beginSequence() {
    ...
    targetTransform.setWorldPosition(START_WORLD_POS);
    setScaleImmediate(START_SCALE);
    baseWorldPos = START_WORLD_POS;
    baseLocalRot = targetTransform.getLocalRotation();
    posOffset = new vec3(0, 0, 0);
    rotOffset = new vec3(0, 0, 0);
    posTarget = new vec3(0, 0, 0);
...
function updateSequence(dt) {
    ...
    if (sequenceState === "ramp") {
        ...
        baseWorldPos = START_WORLD_POS.add(END_WORLD_POS.sub(START_WORLD_POS).uniformScale(smoothT));
        shakeScale = 0.1 + (1.0 - 0.1) * smoothT;
        if (tRamp >= 1) {
            baseWorldPos = END_WORLD_POS;
            shakeScale = 1.0;
            sequenceState = "done";
        }
        return;
...
function update() {
    ...
    if (baseWorldPos === null || baseLocalRot === null) {
        cacheBase();
    }
    ...
    targetTransform.setWorldPosition(baseWorldPos.add(posOffset));
