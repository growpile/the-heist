...
export class WireConnector extends BaseScriptComponent {
  ...
  /** Outer wire group (head/handle sibling space); mesh is built here. */
  private meshSpaceTransform: Transform | null = null
  ...
  initializeWire(): void {
    ...
    this.refreshTransformRefs()
    this.meshSpaceTransform = this.resolveMeshSpaceTransform()

    if (this.headTransform) {
      this.startLocalY = this.headTransform.getLocalPosition().y
      const headWorld = copyVec3(this.headTransform.getWorldPosition())
      this.startHeadPos = headWorld
    }
    ...
    if (this.headTransform && this.startHeadPos) {
      const restHead = addVec3(this.startHeadPos, returnOffset)
      this.headTransform.setWorldPosition(restHead)
      this.startHeadPos = restHead
    }
    if (this.handleTransform && this.startHandlePos) {
      const restHandle = addVec3(this.startHandlePos, returnOffset)
      this.handleTransform.setWorldPosition(restHandle)
      this.startHandlePos = restHandle
    }
    if (this.meshSpaceTransform) {
      if (this.startHeadPos) {
        this.startHeadLocal = worldToLocalPoint(this.meshSpaceTransform, this.startHeadPos)
      }
      if (this.startHandlePos) {
        this.startHandleLocal = worldToLocalPoint(this.meshSpaceTransform, this.startHandlePos)
      }
    }
  ...
  private updateReturn(): void {
    ...
    const restSpace = this.meshSpaceTransform ?? this.baseTransform
    if (this.startHeadLocal && restSpace) {
      targetHead = localToWorldPoint(restSpace, this.startHeadLocal)
    }
    if (this.startHandleLocal && restSpace) {
      targetHandle = localToWorldPoint(restSpace, this.startHandleLocal)
    }
  ...
  private resolveMeshSpaceTransform(): Transform | null {
    if (!this.baseTransform) {
      return null
    }
    const parentObj = this.getSceneObject().getParent()
    if (parentObj) {
      return parentObj.getTransform()
    }
  ...
  private getWireStartWorld(): vec3 {
    ...
    for (let i = 0; i < baseObj.getChildrenCount(); i++) {
      ...
      if (child && child.name === "Wire") {
        return child.getTransform().getWorldPosition()
      }
    }
    return this.baseTransform.getWorldPosition()
  }

  private getWireEndWorld(): vec3 {
    if (this.endTransform) {
      return this.endTransform.getWorldPosition()
    }
    if (this.headTransform) {
      return this.headTransform.getWorldPosition()
    }
  ...
  private getReturnOffset(): vec3 {
    const meshSpace = this.meshSpaceTransform ?? this.resolveMeshSpaceTransform()
    if (!meshSpace) {
      return vec3.zero()
    }
    const meshRot = meshSpace.getWorldRotation()
    return meshRot.multiplyVec3(new vec3(RETURN_OFFSET_DISTANCE, 0, 0))
  }
  ...
  private buildSmoothPath(startPos: vec3, endPos: vec3, segments: number, radius: number): vec3[] {
    ...
    const controlB = new vec3(
      endPos.x - normDir.x * radius * elbowCurvature,
      endPos.y - normDir.y * radius * elbowCurvature,
      endPos.z - normDir.z * radius * elbowCurvature
    )
