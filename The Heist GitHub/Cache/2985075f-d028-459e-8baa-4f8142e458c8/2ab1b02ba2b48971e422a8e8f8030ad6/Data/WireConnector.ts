...
export class WireConnector extends BaseScriptComponent {
  ...
  private bindPublicApi(): void {
    ...
    api.init = () => this.init()
    api.disconnect = (silent?: boolean) => this.disconnect(silent)
    api.disable = () => this.disable()
    api.returnConnections = () => this.returnConnections()
    api.setManager = (manager: ScriptComponent) => this.setManager(manager)
  }
  ...
  const basePos = this.baseTransform.getWorldPosition()
    ...
    if (this.endTransform) {
      const dir = this.endTransform.getWorldPosition().sub(basePos)
      if (dir.length > 1e-5) {
        axis = dir.normalize()
      }
    }
    if (!axis && this.headTransform) {
      const dir = this.headTransform.getWorldPosition().sub(basePos)
      if (dir.length > 1e-5) {
        axis = dir.normalize()
      }
    }
    if (!axis) {
      axis = this.baseTransform.getWorldRotation().multiplyVec3(vec3.up())
    }
    return axis.uniformScale(-1.25)
  }
  ...
  private updatePipe(): void {
    ...
    const endPosWorld = this.endTransform
      ...
      : this.headTransform
        ...
        : null
    if (!endPosWorld || startPosWorld.distance(endPosWorld) < 1e-5) {
      return
    }
