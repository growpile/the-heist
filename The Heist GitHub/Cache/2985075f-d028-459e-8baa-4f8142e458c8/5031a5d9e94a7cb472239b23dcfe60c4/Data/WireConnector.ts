...
export class WireConnector extends BaseScriptComponent {
  ...
  private readInputObject(key: "wireHead" | "wireEnd" | "wireHandle"): SceneObject | null {
    const api = this as unknown as Record<string, SceneObject | undefined>
    return this[key] ?? api[key] ?? null
  }

  /** Resolve @input scene object transforms — inputs may not be wired in onAwake. */
  private refreshTransformRefs(): void {
    if (!this.baseTransform) {
      this.baseTransform = this.getSceneObject().getTransform()
    }
    const head = this.readInputObject("wireHead")
    const end = this.readInputObject("wireEnd")
    const handle = this.readInputObject("wireHandle")
    if (head) {
      this.wireHead = head
      this.headTransform = head.getTransform()
    }
    if (end) {
      this.wireEnd = end
      this.endTransform = end.getTransform()
    }
    if (handle) {
      this.wireHandle = handle
      this.handleTransform = handle.getTransform()
    }
  }
  ...
  initializeWire(): void {
    ...
    if (!this.headTransform || !this.endTransform) {
      print(
        "[WireConnector] Missing wireHead/wireEnd on " +
          this.getSceneObject().name +
          " — tube mesh will not render correctly"
      )
      return
    }
  ...
  triggerDisconnect(silent?: boolean): void {
    if (this.isReturning && !this.isFollowing) {
      return
    }
  ...
  private getWireAxisLocal(): vec3 {
    const len = this.raiseDirection.length
    if (len > 1e-5) {
      return this.raiseDirection.uniformScale(1 / len)
    }
    return vec3.forward()
  }

  /** Pipe mesh lives in base local space; remap so stretch follows raiseDirection (Z), not X. */
  private toWirePathLocal(local: vec3): vec3 {
    return new vec3(local.z, local.y, local.x)
  }
  ...
  private getReturnOffset(): vec3 {
    ...
    return baseRot.multiplyVec3(this.getWireAxisLocal()).uniformScale(1.25)
  }
  ...
  private updatePipe(): void {
    ...
    const startPos = this.toWirePathLocal(worldToLocalPoint(this.baseTransform, startPosWorld))
    const endPos = this.toWirePathLocal(worldToLocalPoint(this.baseTransform, endPosWorld))
