...
function localDirectionToWorld(transform, localDir) {
    if (!transform) {
        return localDir;
    }
    return rotateVec3ByQuat(transform.getWorldRotation(), localDir);
}
...
let WireConnector = (() => {
    ...
    var WireConnector = _classThis = class extends _classSuper {
        ...
        updatePipe() {
            ...
            if (raiseOffset > 0) {
                const raiseWeights = this.getRaisePointWeights(startPosWorld, registryEndWorld, segments);
                ...
                if (raiseKeys.length > 0) {
                    ...
                    if (raiseDirLen > 1e-5) {
                        const raiseDirWorld = localDirectionToWorld(this.baseTransform, raiseDir.uniformScale(1 / raiseDirLen));
                        for (const key of raiseKeys) {
                            const idx = parseInt(key, 10);
                            const weight = raiseWeights[key];
                            points[idx] = points[idx].add(raiseDirWorld.uniformScale(raiseOffset * weight));
                        }
                    }
                }
            }
