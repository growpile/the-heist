...
export class WireConnector extends BaseScriptComponent {
  ...
  private getReturnOffset(): vec3 {
    ...
    return baseRot.multiplyVec3(vec3.up()).uniformScale(-RETURN_OFFSET_DISTANCE)
  }
  ...
  private updatePipe(): void {
    ...
    const meshEndWorld = this.headTransform
      ...
      : this.endTransform
        ...
        : startPosWorld
    const registryEndWorld = this.endTransform
      ? this.endTransform.getWorldPosition()
      : meshEndWorld
    ...
    this.registerWire(startPosWorld, registryEndWorld, segments)

    let points = this.buildSmoothPath(startPosWorld, meshEndWorld, segments, radius)

    if (raiseOffset > 0) {
      const raiseWeights = this.getRaisePointWeights(
        startPosWorld,
        registryEndWorld,
        segments
      )
      ...
      if (raiseKeys.length > 0) {
        ...
        if (raiseDirLen > 1e-5) {
          const raiseDirWorld = localDirectionToWorld(
            this.baseTransform,
            raiseDir.uniformScale(1 / raiseDirLen)
          )
          for (const key of raiseKeys) {
            const idx = parseInt(key, 10)
            const weight = raiseWeights[key]
            points[idx] = points[idx].add(
              raiseDirWorld.uniformScale(raiseOffset * weight)
            )
          }
        }
      }
  ...
  private buildTubeMesh(
    ...
    if (!pathWorld || pathWorld.length < 2 || !this.baseTransform) {
      return
    }
    const totalRings = pathWorld.length
    const vertsPerRing = faceCount + 1
    const meshTransform = this.baseTransform

    for (let i = 0; i < totalRings; i++) {
      const centerWorld = pathWorld[i]
      let forward: vec3
      if (i === 0) {
        forward = pathWorld[i + 1].sub(centerWorld)
      } else if (i === totalRings - 1) {
        forward = centerWorld.sub(pathWorld[i - 1])
      } else {
        forward = pathWorld[i + 1].sub(pathWorld[i - 1])
      }
      ...
      for (let j = 0; j <= faceCount; j++) {
        ...
        const dirWorld = right
          .uniformScale(Math.cos(angle))
          .add(up.uniformScale(Math.sin(angle)))
        const posWorld = centerWorld.add(dirWorld.uniformScale(radius))
        const posLocal = worldToLocalPoint(meshTransform, posWorld)
        const normalLocal = inverseRotateVec3(
          meshTransform.getWorldRotation(),
          dirWorld
        )
        const uv = [j / faceCount, i / (totalRings - 1)]
        builder.appendVertices([
          [posLocal.x, posLocal.y, posLocal.z],
          [uv[0], uv[1]],
