...
export class WireConnector extends BaseScriptComponent {
  ...
  initializeWire(): void {
    ...
    const pipeParent = this.meshSpaceTransform ?? this.baseTransform
    if (pipeParent) {
      this.pipeObj.setParent(pipeParent.getSceneObject())
    }
  ...
  private raiseOffsetInMeshSpace(weight: number): vec3 {
    const meshSpace = this.meshSpaceTransform ?? this.baseTransform
    if (!meshSpace || !this.baseTransform) {
      return vec3.zero()
    }
    ...
    const raiseNorm = raiseDir.uniformScale(1 / raiseDirLen)
    const worldDir = rotateVec3ByQuat(this.baseTransform.getWorldRotation(), raiseNorm)
    const meshDir = inverseRotateVec3(meshSpace.getWorldRotation(), worldDir)
    return meshDir.uniformScale((this.raiseOffset || 0) * weight)
  }
      ...
      if (raiseKeys.length > 0) {
        ...
        if (raiseDirLen > 1e-5) {
          for (const key of raiseKeys) {
            ...
            points[idx] = points[idx].add(this.raiseOffsetInMeshSpace(weight))
          }
        }
      }
    }
