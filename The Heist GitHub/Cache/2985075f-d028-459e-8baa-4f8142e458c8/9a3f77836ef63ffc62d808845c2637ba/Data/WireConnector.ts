...
export class WireConnector extends BaseScriptComponent {
  ...
  private getReturnSlideDistance(): number {
    return Math.max(0, RETURN_OFFSET_TOTAL_SLIDE - PREFAB_HEAD_LEAD_ON_SLIDE)
  }

  /** +X on outer wire parent; only the remaining slide after prefab head lead (~0.5). */
  private applyReturnRestOffset(): void {
    const slideDist = this.getReturnSlideDistance()
    if (slideDist < 1e-5) {
      return
    }
    const returnLocal = new vec3(slideDist, 0, 0)
