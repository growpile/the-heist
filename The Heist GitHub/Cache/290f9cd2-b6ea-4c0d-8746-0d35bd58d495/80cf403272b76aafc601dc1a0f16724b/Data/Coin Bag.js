...
script.play3DParticles = function(callback) {
    ...
    global.utils.animateMaterialProperty(newMaterial, "mainPass.opacityMultiplier", 0, 0.6, function() {
        print('destroy coins now');
        callback();
    });
}

global.destroyCoinsNow = function() {
    print("destroy coins now");
    for (var i = 0; i < spawnedCoins.length; i++) {
        if (spawnedCoins[i]) {
            spawnedCoins[i].destroy();
        }
    }
    spawnedCoins = [];
}
