...
script.play3DParticles = function() {
    for(var i = 0; i < 8; i++) {
        ...
        if (body) {
            ...
            if (body.addForce) {
                if (typeof Physics !== "undefined" && Physics.ForceMode) {
                    var forceModes = [];
                    if (Physics.ForceMode.Impulse !== undefined) { forceModes.push({ name: "Impulse", mode: Physics.ForceMode.Impulse }); }
                    if (Physics.ForceMode.VelocityChange !== undefined) { forceModes.push({ name: "VelocityChange", mode: Physics.ForceMode.VelocityChange }); }
                    if (Physics.ForceMode.Acceleration !== undefined) { forceModes.push({ name: "Acceleration", mode: Physics.ForceMode.Acceleration }); }
                    if (Physics.ForceMode.Force !== undefined) { forceModes.push({ name: "Force", mode: Physics.ForceMode.Force }); }
                    if (forceModes.length > 0) {
                        var pick = forceModes[global.utils.rng(0, forceModes.length - 1)];
                        print("[CoinBag] launch: addForce " + pick.name);
                        body.addForce(velocity, pick.mode);
                    } else {
