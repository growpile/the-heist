...
export class Safe extends BaseScriptComponent {
  ...
  private enableMenuButton(): void {
    global.utils.delay(0.05, () => {
      this.setMenuButtonPressable(true)
    })
  }

  /** Inspector may assign the CapsuleButton script component reference. */
  private resolveMenuButton(): CapsuleButton | null {
    if (!this.menuButton) {
      return null
    }

    const direct = this.menuButton as CapsuleButton
    if (typeof (direct as {inactive?: boolean}).inactive !== "undefined") {
      return direct
    }

    const asScript = this.menuButton as unknown as ScriptComponent
   ...
   */
  private setMenuButtonPressable(pressable: boolean): void {
    const button = this.resolveMenuButton()
    if (!button) {
      return
    }

    const apply = () => {
      const resolved = this.resolveMenuButton()
      if (!resolved) {
        return
      }
      if (pressable) {
