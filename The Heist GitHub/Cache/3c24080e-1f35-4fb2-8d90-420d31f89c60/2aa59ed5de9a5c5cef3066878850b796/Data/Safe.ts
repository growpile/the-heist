...
export class Safe extends BaseScriptComponent {
  ...
  @input
  @hint("Safe mesh + modules only. Scaled away on win/fail exit; keep post-game UI outside this object.")
  @allowUndefined
  ...
  @input
  @allowUndefined
  @hint("Idle pulse — local scale ping-pongs 1↔3; full cycle in 1s.")
  ...
  private failExitRequested = false
  ...
  private handleAllModulesSolved(): void {
    ...
    this.solveSequence!.playWinSequence(this.safeType, () => {
      ...
      if (flow && typeof flow.presentPostGameWin === "function") {
        flow.presentPostGameWin(this.safeType, this.solveElapsedSeconds, this.bombTimer)
        return
      }
  ...
  private startSolveElapsedTimer(): void {
    ...
    this.solveElapsedEvent.bind(() => {
      if (!this.solveStarted || this.postGameShown || this.safeFailedTriggered) {
        return
      }
  ...
  private playFailExplosionAndScaleAway(onComplete: () => void): void {
    const flow = this.gameFlow as {
