...
export class CoopNetworkController extends BaseScriptComponent implements CoopNetworkFacade {
  ...
  @input
  debugDisplayTexture = false

  @input
  @allowUndefined
  @showIf("debugDisplayTexture", true)
  ...
  @input
  @showIf("useCrop", true)
  cropValue = new vec4(0, 0, 0, 0)

  @input
  downsampleTexture = true

  @input
  @showIf("downsampleTexture", true)
  maxResolution = 250

  @input
  @hint("0 = max compression, 4 = max quality.")
  ...
  @input
  @hint("Frames per second sent over realtime (keep low for 250KB message limit).")
  targetStreamFps = 10
  ...
  private streamLastTime = -1e9
  private streamIntervalSec = 1 / 10
  private streamFrameRegistration: number | null = null
  ...
  /** Start AR glasses encode + realtime broadcast (call when online game begins). */
  startCameraStream(): void {
    const handler = this.cameraAccessHandler as CameraAccessHandlerApi | null
    if (!handler || typeof handler.startStreaming !== "function") {
      this.log("Camera Access Handler missing or has no startStreaming — assign on Realtime Manager")
      return
    }

    this.textureBroadcastEnabled = true
    handler.startStreaming.call(handler, {
      externalScript: this as unknown as ScriptComponent,
      functionName: "onCameraFrameEncoded",
      output: 1,
      targetFps: this.targetStreamFps,
      encodingType: 1,
      compressionQuality: 0,
  ...
  stopCameraStream(): void {
    ...
    const handler = this.cameraAccessHandler as CameraAccessHandlerApi | null
    if (handler && typeof handler.stopStreaming === "function") {
      handler.stopStreaming.call(handler)
    }
    this.log("AR camera stream stopped")
  }

  /** Invoked by Camera Access Handler each encoded frame. */
  onCameraFrameEncoded(encodedString: string): void {
