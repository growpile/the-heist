...
export class CoopNetworkController extends BaseScriptComponent implements CoopNetworkFacade {
  ...
  @input
  @hint("Frames per second drained from buffer and sent over realtime (keep low for 250KB limit).")
  targetStreamFps = 10

  @input
  @hint("Max encoded frames queued before dropping oldest (live AR keeps newest).")
  maxFrameBuffer = 3
  ...
  private onCameraFrame(): void {
    ...
    const now = getTime()
    const bypassThrottle = this.pendingReencode
  ...
  private enqueueEncodedFrame(encoded: string): void {
    ...
    const maxBuffer = Math.max(1, this.maxFrameBuffer || 3)
