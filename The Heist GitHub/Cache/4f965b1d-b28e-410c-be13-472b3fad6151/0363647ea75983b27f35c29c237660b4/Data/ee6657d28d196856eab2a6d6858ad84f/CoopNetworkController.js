...
/** Snap Cloud realtime room, spectators, and AR camera stream. */
let CoopNetworkController = (() => {
    ...
    var CoopNetworkController = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.maxEncodeFps = this.maxEncodeFps;
            ...
            this.streamPumpEvent = null;
            this.encodedFrameBuffer = [];
            this.lastSendTime = -1e9;
            this.lastEncodeStartTime = -1e9;
            this.sendIntervalSec = 1 / 10;
            this.encodeIntervalSec = 1 / 15;
            ...
            this.MAX_GUESTS = 4;
            this.REALTIME_FRAME_LIMIT_BYTES = 250000;
        }
        __initialize() {
            super.__initialize();
            this.snapCloudRequirements = this.snapCloudRequirements;
        ...
        startCameraStream() {
            ...
            const sendFps = Math.max(1, this.targetStreamFps || 24);
            const encodeFps = Math.max(1, this.maxEncodeFps || 24);
            ...
            this.log("AR camera stream started (encode max " +
                encodeFps +
                " fps, send " +
                sendFps +
                " fps, buffer " +
        ...
        beginEncode(texture, encodeStartTime) {
            ...
            return new Promise((resolve, reject) => {
                try {
                    if (Base64.encodeTextureAsync) {
                        Base64.encodeTextureAsync(texture, resolve, reject, STREAM_COMPRESSION_QUALITY, STREAM_ENCODING_TYPE);
                    }
                    else {
                        const syncEncode = Base64
                            .encodeTexture;
                        if (syncEncode) {
                            resolve(syncEncode(texture, STREAM_COMPRESSION_QUALITY, STREAM_ENCODING_TYPE));
                        }
                        else {
