...
let GameFlowController = (() => {
    ...
    var GameFlowController = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.uiKitSoundMaster = this.uiKitSoundMaster;
        ...
        onSettingsTabSelected() {
            ...
            this.uiKitSoundMaster?.updateState();
        ...
        handleSafeComplete(safeType, solvedInSeconds) {
            ...
            const bombTimerSeconds = typeof this.activeSafeComponent?.getBombTimerSeconds === "function"
                ...
                : 0;
            const penaltyCount = typeof this.activeSafeComponent?.getPenaltyCount === "function"
                ? this.activeSafeComponent.getPenaltyCount()
                : 0;
        ...
        handleSafeFailed() {
            ...
            this.logFlow("handleSafeFailed — post-game UI");
            this.presentPostGameFail();
        }
        /** Win: scale safe away, then show MenuController post-game panel. */
        presentPostGameWin(safeType, solveDurationSeconds, bombTimerSeconds, penaltyCount) {
            this.stopTutorialHint();
            this.setRotationEnabled(false);
            this.activeSafeComponent?.endSession();
            const prefabRoot = global.appState.safe?.object;
        ...
        exitGameplaySession(afterMenu) {
            this.logFlow("exitGameplaySession appState=" +
        ...
        syncSettingsControlsFromStorage(_phase) {
        ...
        /** Persist volume and sync BGM + UIKit button audio. */
        ...
        replayTutorial() {
            ...
            this.logFlow("replayTutorial → tutorial tips");
            global.playSfx(1, 1, global.appState.checkStorage("masterVolume") * 1);
        ...
        stopTutorialHint() {
            ...
            if (ctrl.animationPlayer) {
                const player = ctrl.animationPlayer;
        ...
        setRotationEnabled(enabled) {
            ...
            if (this.rotationManager) {
                this.logFlow("rotationManager missing setCanRotate");
            }
        }
        safeIntro(safeType) {
        ...
        async beginNetworkFlow() {
            ...
            try {
                ...
                this.logFlow("createNewRoom exception: " + e);
                code = null;
            }
            if (generation !== this.networkFlowGeneration) {
                return;
        ...
        networkFlowFailed() {
            this.networkFlowGeneration++;
            this.logFlow("network flow failed — return to menu");
            global.appState.inTransition = false;
            this.surfacePlacementActive = false;
            const coop = this.coop();
