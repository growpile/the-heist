...
function computeTubeRingFrame(tangent, prevRight, prevUp, referenceUp) {
...
let WireConnector = (() => {
    ...
    var WireConnector = _classThis = class extends _classSuper {
        ...
        getReturnOffset() {
            ...
            return copyVec3(this.baseTransform
                .getWorldRotation()
                .multiplyVec3(vec3.up())
                .uniformScale(-RETURN_OFFSET_DISTANCE));
        }
        getReturnOffsetInBaseLocal() {
            if (!this.baseTransform) {
                return vec3.zero();
            }
            const returnOffset = this.getReturnOffset();
            const baseWorld = copyVec3(this.baseTransform.getWorldPosition());
            return subVec3(worldToLocalPoint(this.baseTransform, addVec3(baseWorld, returnOffset)), worldToLocalPoint(this.baseTransform, baseWorld));
        }
