 ...
 * Runtime values are set by AppState.ts, Utils.ts, SoundManager.ts, Tween Manager.
 *
...
interface HeistAppState {
  ...
  /** Typed shortcut — avoids broad overload ambiguity at call sites. */
...
interface HeistUtils {
  ...
  cancelObjectAnimations(sceneObject: SceneObject): void
...
declare namespace global {
  ...
  let playSfx: (id: number, times: number, volume: number, soundId?: string) => void
  let stopSfx: (soundId: string) => void
  let crossfadeBgm: (id: number, volume: number, crossfadeDuration: number) => void
  let setBgmVolume: (volume: number, fadeDuration?: number) => void
