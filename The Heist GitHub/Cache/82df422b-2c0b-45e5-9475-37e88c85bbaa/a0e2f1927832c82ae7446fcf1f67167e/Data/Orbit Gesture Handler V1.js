...
var HISTORY_LIMIT = 2; // shorter history for faster stability detection
var DEG2RAD = Math.PI / 180;
var YAW_PER_CM = 3; // degrees per cm slide along camera right (faster orbit)
var ROTATE_LERP_SPEED = 8;
var HORIZONTAL_THRESHOLD = 15; // cm in XZ plane to trigger orbit
var VERTICAL_THRESHOLD = 6; // cm in Y to trigger orbit
...
function startOrbit(hand) {
  ...
  isOrbiting = true;
  activeHand = hand;
  var startPos = hand.getPalmCenter();
  var handKey = hand.handType === "right" ? "right" : "left";
  prevHandPositions[handKey] = startPos;
  orbitPrevPositions[handKey] = startPos;
  orbitStartHandPos = startPos;
  yawOriginHandPos = startPos;
...
function updateOrbit() {
  ...
  var handKey = activeHand.handType === "right" ? "right" : "left";
  var prevPos = orbitPrevPositions[handKey];
  var speed =
    prevPos && getDeltaTime() > 0
      ? currentPos.distance(prevPos) / getDeltaTime()
      : 0;
  orbitPrevPositions[handKey] = currentPos;
...
function checkHand(hand, history, wasClose) {
  ...
  if (canUse) {
    ...
    var prev = prevHandPositions[handKey];
    if (!prev) {
      prevHandPositions[handKey] = palmCenter;
      prev = palmCenter;
    }
    if (prev && getDeltaTime() > 0) {
      speed = palmCenter.distance(prev) / getDeltaTime();
    }
    prevHandPositions[handKey] = palmCenter;
  }
  var flat = canUse && isHandFlat(hand);
  // var still = canUse && !isHandMoving(history); // disabled for testing
  var still = true;
  var faceDown = canUse && isHandWithinAngleThreshold(hand) && flat && still;

  var planarDistance = null;
  var yDistance = null;
  if (palmCenter) {
    var delta = palmCenter.sub(targetPos);
    planarDistance = Math.sqrt(delta.x * delta.x + delta.z * delta.z);
    yDistance = Math.abs(delta.y);
  }
  var isClose =
    planarDistance !== null
      ? planarDistance < HORIZONTAL_THRESHOLD && yDistance < VERTICAL_THRESHOLD
      : false;
  debugState[handKey] = {
    tracked: tracked,
    faceDown: faceDown,
