...
function updateOrbit() {
  ...
  if (rotationStopSpeed && speed > rotationStopSpeed) {
    // re-center orbit origin without applying rotation
    orbitStartHandPos = currentPos;
...
function checkHand(hand, history, wasClose) {
  ...
  var speed = 0;
  if (orbitPrevHandPos && canUse && getDeltaTime() > 0) {
    speed = hand.getPalmCenter().distance(orbitPrevHandPos) / getDeltaTime();
  }
...
function updateDebug() {
  ...
  var speedLine = orbitPrevHandPos
    ? "Speed: " +
      (debugState.right.speed ? debugState.right.speed.toFixed(1) : "0")
    : "Speed: -";
  var lines = [];
  lines.push(
