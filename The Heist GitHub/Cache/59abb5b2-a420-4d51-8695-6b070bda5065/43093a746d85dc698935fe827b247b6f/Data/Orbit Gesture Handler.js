...
var dotCompleteThreshold = 0.995;
...
function startRotation(targetState) {
  ...
  if (!targetTransform || !anchorRotation) {
    rotating = false;
    return;
  }
  var angle = snapState * 90 * DEG2RAD;
  targetRotation = anchorRotation.multiply(
    quat.fromEulerVec(new vec3(0, angle, 0))
  );
...
function updateRotation() {
  ...
  var currN = normalizeQuat(currentRotation);
  var targN = normalizeQuat(targetRotation);
  var d = quatDot(currN, targN);
  if (d > dotCompleteThreshold) {
    targetTransform.setWorldRotation(targetRotation);
    anchorRotation = targetTransform.getWorldRotation(); // use new orientation as anchor for next snaps
    rotating = false;
...
function update() {
  ...
  if (hand && hand.isTracked()) {
    ...
    // Trigger orbit
    if (!orbitActive && !rotating && faceDown && distance < script.minDistance && yDist < 4 ) {
      startOrbit(hand);
    }
