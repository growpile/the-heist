...
// Lightweight orbit snapper: detect face-down, flat, still hand within range,
...
function addHistory(hand, key) {
  if (!histories[key]) {
    histories[key] = [];
  }
  histories[key].push(hand.thumbTip.position);
  if (histories[key].length > HISTORY_LIMIT) {
    histories[key].shift();
  }
}
...
function isMoving(key) {
  ...
  var movement = hist[0].distance(hist[hist.length - 1]);
...
function updateHandDebug(hand, key, faceDown, distance, yDist) {
  ...
  lines.push("Still: " + (!isMoving(key) ? "true" : "false"));
...
function update() {
  ...
  if (hand && hand.isTracked()) {
    ...
    var faceDown = flat && still && angleOk;
