...
script.createEvent("OnStartEvent").bind(async function() {
    print("Testing CAH: starting OnStartEvent");

    // Event-based test: capture frames with refresh and display
    var handler = (cameraAccessHandler2 && cameraAccessHandler2.api) ? cameraAccessHandler2.api : cameraAccessHandler2;
    if (!handler || !handler.captureFrame) {
        print("Testing CAH: cameraAccessHandler2 missing or captureFrame not available");
        return;
    }

    var ev = handler.captureFrame()
        .fromCamera(0)
        .withResolution(400)
        .withRefreshRate(0.2)
        .get();
    ...
    if (ev && ev.add) {
        print("Testing CAH: event mode active");
        ev.add(function(tex) {
            if (debugImage) {
        ...
        print("Testing CAH: event not available, falling back to single capture");
