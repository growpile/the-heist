...
function generateRules() {
    ...
    // if the second-to-last wire is COLOR.

    // MODIFIERS
    // Cut from left to right. (only once)
    // The next rule is a lie. (only once)
    // The previous rule is a lie. (only once)
    // Do not cut any wires ajdacent to COLOR wires. (only once)
    // Do not cut any wires ajdacent to odd/even wires. (only once)
    // Other rules colors are swapped. When they say COLOR1, they mean COLOR2. (only once)
    // Disregard other rules if there is a COLOR wire. Cut it. (only once)
}
