...
function generateRules() {
    ...
    // UNIQUE (can't apply IFS or MODIFIERS here). These are more complex standalone rules.
    ...
    // MODIFIERS (only 2 of these can per round)
    // Cut from left to right. This rule disregards order priority. (only once)
    // The next/previous rule is a lie. (only once)
    // Do not cut any wires adjacent to COLOR wires./Do not cut any wires adjacent to odd/even wires. (only once)
    //  (only once)
