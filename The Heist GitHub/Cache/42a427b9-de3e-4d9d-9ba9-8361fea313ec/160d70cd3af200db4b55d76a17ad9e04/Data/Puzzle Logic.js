...
function createRules() {
    ...
    solution = new Array(activeWiresCount);
    keepLocks = new Array(activeWiresCount);
    for (var fillIdx = 0; fillIdx < activeWiresCount; fillIdx++) {
        solution[fillIdx] = false;
        keepLocks[fillIdx] = false;
    }
    colorAlias = { red: "red", green: "green", blue: "blue", yellow: "yellow" };
    plannedCutColors = {};
    cutCount = 0;
    ...
    if (randomInt(0, 1) === 1) {
        var uniqueRule = sample(uniqueFactories)();
        ruleEntries.push({ rule: uniqueRule, isUnique: true });
    }

    var standardCount = 3;
    for (var s = 0; s < standardCount; s++) {
        ruleEntries.push({ rule: sample(standardFactories)(), isUnique: false });
    }
    ...
    for (var r = 0; r < ruleEntries.length; r++) {
        ...
        if (!entry.isUnique) {
            if (ifCount < 3 && randomInt(0, 1) === 1) {
                var availableIfs = ifFactories.filter(function(factory) {
                    var temp = factory();
                    return !usedIfs[temp.id];
                });
                if (availableIfs.length > 0) {
                    var cond = sample(availableIfs)();
                    usedIfs[cond.id] = true;
                    ifCount++;
                    entry.condition = cond;
                }
            }
            if (modCount < 2 && randomInt(0, 1) === 1) {
                var availableMods = modifierFactories.filter(function(factory) {
                    var temp = factory();
                    return !usedModifiers[temp.id];
                });
                if (availableMods.length > 0) {
                    var mod = sample(availableMods)();
                    usedModifiers[mod.id] = true;
                    modCount++;
                    entry.modifier = mod;
                }
            }
    ...
    for (var ri = 0; ri < ruleEntries.length; ri++) {
        var current = ruleEntries[ri];

        if (current.isUnique) {
            var msg = current.rule.applyUnique();
            ruleLines.push("- " + current.rule.description + " [Standalone]");
            lastInstruction = null;
            continue;
        }

        if (current.modifier && current.modifier.id === Modifiers.CutRightToLeft) {
            solveDirection = ReadDirection.RightToLeft;
        }
        if (current.modifier && current.modifier.id === Modifiers.SwapColors) {
            swapColors(current.modifier.data.from, current.modifier.data.to);
        }

        var ctx = buildRuleContext();
        var conditionMet = true;
        var label = current.rule.description;
        if (current.condition) {
            label += " if " + current.condition.text;
            conditionMet = current.condition.predicate(ctx);
        }
        ...
        var instruction = current.rule.createInstruction();
        if (!instruction) {
        ...
        if (invertNext) {
            instruction = invertInstruction(instruction);
            invertNext = false;
        }
        ...
        if (current.modifier) {
            if (current.modifier.id === Modifiers.NextRuleIsLie) {
                invertNext = true;
            } else if (current.modifier.id === Modifiers.PreviousRuleIsLie && lastInstruction) {
                var invertedPrev = invertInstruction(lastInstruction);
                applyInstruction(invertedPrev);
                lastInstruction = invertedPrev;
            } else if (current.modifier.id === Modifiers.DoNotCutAdjacent) {
                var adjMask = new Array(activeWiresCount);
                for (var a = 0; a < activeWiresCount; a++) adjMask[a] = false;
                for (var b = 0; b < activeWiresCount; b++) {
                    var match = false;
                    if (current.modifier.data.color) {
                        match = wires[b].color === mapColor(current.modifier.data.color);
                    } else if (current.modifier.data.parity) {
                        match = isIndexParity(b, current.modifier.data.parity);
                    }
                    if (match) {
            ...
            } else if (current.modifier.id === Modifiers.DoNotCutColor) {
                applyInstruction(makeInstruction({
                    type: InstructionType.Prohibitive,
                    indexes: collectIndexesByColor(current.modifier.data.color),
                    message: "Do not cut " + current.modifier.data.color + " wires",
                    size: activeWiresCount,
                    lockKeep: true
                }));
            }
        ...
        ruleLines.push("- " + label + (current.modifier ? " (" + current.modifier.text + ")" : ""));
    }
