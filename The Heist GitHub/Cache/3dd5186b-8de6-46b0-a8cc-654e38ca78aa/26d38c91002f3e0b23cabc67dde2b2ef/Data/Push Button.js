...
function checkHand(hand, center, half, axisWorld) {
    ...
    for (var i = 0; i < tips.length; i++) {
        var tip = tips[i];
        var offset = tip.pos.sub(center);
        if (axisWorld && offset.dot(axisWorld) < 0) {
            continue;
        }
...
function onUpdate() {
    ...
    var halfDepth = axisConfig.axisSize * 0.5;
    var frontPlane = center.add(axisConfig.axisWorld.uniformScale(halfDepth));

    var anyHit = checkHand(leftHand, center, half, axisConfig.axisWorld) ||
        checkHand(rightHand, center, half, axisConfig.axisWorld);
    ...
    if (anyHit) {
        ...
        for (var h = 0; h < hands.length; h++) {
            ...
            for (var t = 0; t < tips.length; t++) {
                ...
                if (offset.dot(axisConfig.axisWorld) < 0) { continue; }
                if (!isInsideBounds(tips[t], center, half)) { continue; }
                var depth = frontPlane.sub(tips[t]).dot(axisConfig.axisWorld); // positive when pushing inward past front plane
