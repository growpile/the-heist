...
export class SurfaceDetector extends BaseScriptComponent {
  ...
  protected onCalibrationComplete(callback: () => void) {
    ...
    delay.reset(0.1);
    //play audio
    const audioComponent = this.getSceneObject()
      .getParent()
      .getComponent("Component.AudioComponent") as AudioComponent | null;
    if (audioComponent) {
      const masterVolume =
        global.appState && typeof global.appState.checkStorage === "function"
          ? Number(global.appState.checkStorage("masterVolume") || 0)
          : 1;
      audioComponent.volume = Math.max(0, Math.min(1, masterVolume));
