...
global.utils.animateShake = function(
    ...
    var targetObject = global.utils.shakeTarget || global.utils.lastAnimatedObject;
    if (!targetObject) {
        if (onDone) { onDone(); }
        return;
    }
    ...
    var baseLocalPos = transform.getLocalPosition();
    var baseLocalRot = transform.getLocalRotation();
    ...
    animationData.updateEvent.bind(function() {
        ...
        if (!ending) {
            ...
            if (donePos && doneRot) {
                transform.setLocalPosition(baseLocalPos);
                transform.setLocalRotation(baseLocalRot);
                if (animationData.cleanup) {
                    animationData.cleanup();
                }
                animationData.updateEvent.enabled = false;
        ...
        var finalPos = baseLocalPos;
        if (positionShake) {
            finalPos = baseLocalPos.add(posOffset);
        }
