...
// Animate any numeric material property (e.g. animateMaterialProperty(mat, "mainPass.progress", 1, 0.25))
global.utils.animateMaterialProperty = function(material, propertyString, targetValue, duration, callback) {
    if (!material || !propertyString) {
        if (callback) { callback(); }
        return;
    }
    var root = material;
    var parts = String(propertyString).split(".");
    ...
    if (!material.__materialAnims) {
        material.__materialAnims = {};
    }
    var animKey = String(propertyString);
    var existing = material.__materialAnims[animKey];
    ...
    material.__materialAnims[animKey] = animData;
