...
var streamRegistration = null;
var isStreaming = false;
var streamLastTime = -1e9;
var streamIntervalSec = 1 / 15;
var streamingCameraTexture = null;
...
script.startStreaming = function(options) {
    ...
    if (opts.compressionQuality !== undefined) {
        script.compressionQuality = opts.compressionQuality;
    } else {
        script.compressionQuality = 0;
    }

    var fps = opts.targetFps !== undefined ? opts.targetFps : (opts.fps !== undefined ? opts.fps : 15);
    fps = Math.max(1, fps);
    streamIntervalSec = 1 / fps;

    var includeRendered = (opts.includeRenderedContent !== undefined) ? opts.includeRenderedContent : true;

    if (!streamingProvider || !streamingCameraTexture) {
        var req = CameraModule.createCameraRequest();
        handleCameraRequest(req, opts.optionalConfig);
        try {
            var camTex = cameraModule.requestCamera(req);
            streamingCameraTexture = camTex;
            latestTexture = camTex;
            streamingProvider = camTex.control;
            cameraTextureProvider = streamingProvider;
        } catch (e) {
            printError(e);
            return;
        }
    ...
    if (streamRegistration && streamingProvider && streamingProvider.onNewFrame) {
        streamingProvider.onNewFrame.remove(streamRegistration);
        streamRegistration = null;
    }

    if (!streamingProvider || !streamingProvider.onNewFrame) {
        printError("Camera provider not available for streaming");
        return;
    }
    ...
    streamRegistration = streamingProvider.onNewFrame.add(function(frame) {
        if (!isStreaming) { return; }

        var now = getTime();
        if (now - streamLastTime < streamIntervalSec) { return; }
        streamLastTime = now;

        var baseTexture = streamingCameraTexture || latestTexture;
        if (!baseTexture) { return; }

        var finalTexture = baseTexture;
        if (includeRendered && script.screenCropTexture && script.screenCropTexture.control && script.compositeTexture) {
            script.screenCropTexture.control.inputTexture = baseTexture;
            if (!compositeCameraSetup && script.virtualRenderCameraSetup) {
                compositeCameraSetup = script.virtualRenderCameraSetup.instantiate(script.getSceneObject());
            }
