...
function handleCameraRequest(cameraRequest) {
    ...
    if(script.downsampleTexture) script.maxResolution != 0 ? script.maxResolution : 200;
    script.downsampleTexture ? cameraRequest.imageSmallerDimension = script.maxResolution;
}

script.createEvent('OnStartEvent').bind(() => {
    // create & configure request based on inspector input
    var request = script.input == 0 ? CameraModule.createCameraRequest() : CameraModule.createImageRequest();
