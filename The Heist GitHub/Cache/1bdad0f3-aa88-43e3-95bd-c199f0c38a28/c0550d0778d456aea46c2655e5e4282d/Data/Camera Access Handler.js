...
function handleImageRequest(imageRequest) {
    ...
    if (script.customResolution && script.resolution) {
        imageRequest.captureWidth = script.resolution.x;
        imageRequest.captureHeight = script.resolution.y;
    }
    if (script.useCrop && script.cropValue) {
        // cropValue: x,y,width,height normalized (0-1) or pixels depending on Lens Studio API
        imageRequest.cropRect = script.cropValue;
    }
}
