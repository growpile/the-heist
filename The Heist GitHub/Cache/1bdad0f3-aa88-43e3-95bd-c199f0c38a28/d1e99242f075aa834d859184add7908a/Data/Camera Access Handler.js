...
function sendTextureIfNeeded(tex) {
    if (!tex) { return; }
    if (script.output === 0 && script.externalScript && script.functionName && script.externalScript[script.functionName]) {
...
function initUpdateEvent() {
    ...
    updateEvent.bind(function() {
        var texToUse = compositeRenderTexture || latestTexture;
        if (!texToUse) { return; }

        // If output is texture, just forward it
        if (script.output === 0) {
            sendTextureIfNeeded(texToUse);
            return;
        }

        // Otherwise, encode on interval
        var now = getTime();
