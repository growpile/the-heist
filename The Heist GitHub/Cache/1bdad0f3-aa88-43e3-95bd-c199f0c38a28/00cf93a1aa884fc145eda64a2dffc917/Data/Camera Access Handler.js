...
async function requestOutput(request) {
    var outputTexture = script.input == 0 ? await CameraModule.requestImage(request) : await CameraModule.requestCamera(request);
...
script.createEvent('OnStartEvent').bind(() => {
    ...
    var outputTexture = requestOutput(request);
    
    var textureProvider = outputTexture.control;
});
