...
function handleImageRequest(imageRequest) {
    if (script.customResolution && script.resolution.x != 0 && script.resolution.y != 0) imageRequest.resolution = new vec2(script.resolution.x, script.resolution.y);
    if (!script.useCrop) return;
...
// Exposed capture functions ---------------------------------------------------
script.captureCameraFrame = function(callback, optionalConfig) {
    var req = CameraModule.createCameraRequest();
    handleCameraRequest(req);
    if (optionalConfig) {
        if (optionalConfig.optionalCameraIdInt !== undefined && optionalConfig.optionalCameraIdInt !== null) {
            req.cameraId = optionalConfig.optionalCameraIdInt;
        }
        if (optionalConfig.optionalMaxResolutionInt !== undefined && optionalConfig.optionalMaxResolutionInt !== null) {
            req.imageSmallerDimension = optionalConfig.optionalMaxResolutionInt;
        }
    }
    try {
...
script.captureCameraImage = async function(callback, optionalConfig) {
    var req = CameraModule.createImageRequest();
    handleImageRequest(req);
    if (optionalConfig) {
        if (optionalConfig.optionalCustomResolutionVec2) {
            var r = optionalConfig.optionalCustomResolutionVec2;
            if (r.x && r.y) {
                req.resolution = new vec2(r.x, r.y);
            }
        }
        if (optionalConfig.optionalCropVec4) {
            var c = optionalConfig.optionalCropVec4;
            req.cropRect = Rect.create(c.x, c.y, c.z, c.w);
        }
    }
    try {
        var imageFrame = cameraModule.getImageFrameAsync
        ...
        if (callback) { callback(latestTexture); }
