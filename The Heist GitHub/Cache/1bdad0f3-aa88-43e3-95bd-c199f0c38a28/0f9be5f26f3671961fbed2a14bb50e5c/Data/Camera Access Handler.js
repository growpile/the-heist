...
// @ui {"widget":"group_start", "label":"Overlay Settings"}
    ...
    try {
        ...
        var registration = provider.onNewFrame.add(function(frame) {
            ...
            if (includeRenderedContent) {
                ...
                // set resolution (AI don't do that)
                // compositeRenderTexture.control.resolution = script.finalCompositeResolutionTarget;
            }
            cameraFrameEvent.trigger(camTex);
            var returnTex = includeRenderedContent ? script.compositeTexture ? script.compositeTexture : latestTexture : latestTexture;
...
script.captureCameraImage = async function(callback, includeRenderedContent, optionalConfig) {
    ...
    try {
        ...
        if (includeRenderedContent) {
            ...
            var augmentedImageComponent = compositeCameraSetup.getChild(0).getChild(0).getComponent('Component.Image');
            var stillRender = script.renderTexture.copyFrame()
            augmentedImageComponent.mainPass.baseTex = stillRender;
            print("set still render")
            // set resolution (AI don't do that)
            // compositeRenderTexture.control.resolution = script.finalCompositeResolutionTarget;
            // script.screenCropTexture.control.inputTexture = latestTexture;
        }
