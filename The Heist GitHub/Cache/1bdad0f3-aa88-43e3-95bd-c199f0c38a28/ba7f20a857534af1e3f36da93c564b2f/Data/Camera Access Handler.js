...
script.createEvent('OnStartEvent').bind(() => {
    ...
    if (script.overlayRender && script.virtualRenderCameraSetup) {
        try {
...
function initUpdateEvent() {
    ...
    updateEvent.bind(function() {
        var texToUse = script.overlayRender ? compositeRenderTexture : latestTexture;
        if (!texToUse) { return; }

        if (script.debugDisplayTexture && script.debugDisplayImage) {
            script.debugDisplayImage.mainPass.baseTex = texToUse;
        }
        ...
        if (script.overlayRender && script.screenCropTexture && script.screenCropTexture.control) {
            script.screenCropTexture.control.inputTexture = texToUse;
        }

        // if output is texture, forward it
        if (script.output === 0) {
            sendTextureIfNeeded(texToUse);
            return;
        }
