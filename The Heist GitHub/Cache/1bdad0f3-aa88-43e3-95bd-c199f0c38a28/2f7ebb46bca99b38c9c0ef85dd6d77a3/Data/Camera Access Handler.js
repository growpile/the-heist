...
// @input bool includeRenderedContent = false {"showIf":"overlayRender"}

// hidden
// @input Asset.ObjectPrefab virtualRenderCameraSetup
// @input Asset.Texture screenCropTexture
// @input Asset.Texture compositeStreamTexture
...
var compositeStreamTexture = script.compositeStreamTexture;
...
script.captureCameraFrame = function(callback, includeRenderedContent, optionalConfig) {
    ...
    try {
        ...
        var registration = provider.onNewFrame.add(function(frame) {
            ...
            var returnTex = script.overlayRender && compositeStreamTexture ? compositeStreamTexture : camTex;
            if (callback) { callback(returnTex); }
        });
        return script.overlayRender && compositeStreamTexture ? compositeStreamTexture : camTex;
    } catch (e) {
        printError(e);
...
script.captureCameraImage = async function(callback, includeRenderedContent, optionalConfig) {
    ...
    try {
        ...
        var returnTex = script.overlayRender && compositeStreamTexture ? compositeStreamTexture : latestTexture;
        if (callback) { callback(returnTex); }
        return returnTex;
    } catch (e) {
        printError(e);
...
function initUpdateEvent() {
    ...
    updateEvent.bind(function() {
        var finalTexture = script.overlayRender ? compositeStreamTexture : latestTexture;
