    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            if (Base64.encodeTextureAsync) {
                Base64.encodeTextureAsync(texture, function(str){ resolve(str); }, function(err){ reject(err); }, q, e);
            } else if (Base64.encodeTexture) {
                // Some builds only accept the texture argument synchronously
                var result = Base64.encodeTexture(texture);
                resolve(result);
            } else {
