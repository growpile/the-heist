...
/** API exposed to WireConnector instances (replaces legacy script.* on fusebox). */
...
export class WireFuseboxModule extends BaseScriptComponent {
  ...
  onAwake(): void {
    const self = this as unknown as WireFuseboxManager
    ;(this as unknown as Record<string, unknown>).registerSockets = (sockets) =>
      this.registerSockets(sockets)
    ;(this as unknown as Record<string, unknown>).unregisterSockets = (sockets) =>
      this.unregisterSockets(sockets)
    ;(this as unknown as Record<string, unknown>).isSocketOccupied = (socket) =>
      this.isSocketOccupied(socket)
    ;(this as unknown as Record<string, unknown>).occupySocket = (socket, wire) =>
      this.occupySocket(socket, wire)
    ;(this as unknown as Record<string, unknown>).releaseSocket = (wire) =>
      this.releaseSocket(wire)
    ;(this as unknown as Record<string, unknown>).getOccupancy = () => this.getOccupancy()
    ;(this as unknown as Record<string, unknown>).getSockets = () => this.getSockets()
  }
