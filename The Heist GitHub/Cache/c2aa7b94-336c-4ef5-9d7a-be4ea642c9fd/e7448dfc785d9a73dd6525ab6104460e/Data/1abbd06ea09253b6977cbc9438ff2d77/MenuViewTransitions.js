...
class MenuViewTransitions {
    ...
    popInRootThenElements(root, elements, onComplete) {
        ...
        this.popInElement(shell, shell.getTransform(), this.getRestScale(shell), 0, () => {
            shellDone = true;
            tryFinish();
        }, this.shellPopInOvershootDurationMs, this.shellPopInSettleDurationMs, false);
        if (content.length === 0) {
            return;
        }
    ...
    popInChildElementsWithBaseDelay(elements, baseDelayMs, onComplete) {
        this.animateElementsStaggered(elements, (element, transform, restScale, startDelayMs, onElementComplete) => {
            const revealDescendants = !/^(solo|coop)\s*view$/i.test(element.name.trim());
            this.popInElement(element, transform, restScale, baseDelayMs + startDelayMs, onElementComplete, this.childPopInOvershootDurationMs, this.childPopInSettleDurationMs, revealDescendants);
        }, onComplete);
    }
    ...
    splitShellAndContent(elements) {
        ...
        const shellNamePattern = /panel|window|frame|plate|shell/i;
        let shellIndex = -1;
        let bestArea = -1;
        let largestIndex = 0;
        ...
        if (shellIndex < 0) {
            for (let i = 0; i < elements.length; i++) {
                const element = elements[i];
                const name = element.name.trim();
                if (containerViewPattern.test(name)) {
                    shellIndex = i;
                    break;
                }
                if (shellNamePattern.test(name)) {
                    shellIndex = i;
                    break;
                }
    ...
    popInElement(element, transform, restScale, startDelayMs, onComplete, overshootDurationMs, settleDurationMs, revealDescendants = true) {
        element.enabled = true;
        if (revealDescendants) {
            this.setDescendantsEnabled(element, true);
        }
