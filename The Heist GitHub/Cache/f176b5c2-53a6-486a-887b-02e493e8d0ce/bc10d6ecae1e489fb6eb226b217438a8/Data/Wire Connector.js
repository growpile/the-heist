...
function clampToPositiveLocalX(pos) {
    if (!headTransform || startLocalX === null) {
        return pos;
    }
    var parentObj = headTransform.getSceneObject().getParent();
    if (!parentObj) {
        return pos;
    }
    var parentTransform = parentObj.getTransform();
    var parentPos = parentTransform.getWorldPosition();
    var parentRot = parentTransform.getWorldRotation();
    var parentRotInv = invertQuat(parentRot);
    if (!parentRotInv) {
        return pos;
    }
    var local = parentRotInv.multiplyVec3(pos.sub(parentPos));
    if (local.x < startLocalX) {
        local.x = startLocalX;
    }
...
function applyConstraints(pos) {
    var clamped = clampToPositiveLocalX(pos);
...
function init() {
    if (headTransform) {
        ...
        if (parentObj) {
            var parentTransform = parentObj.getTransform();
            var parentPos = parentTransform.getWorldPosition();
            var parentRotInv = invertQuat(parentTransform.getWorldRotation());
            if (parentRotInv) {
                var headLocal = parentRotInv.multiplyVec3(headTransform.getWorldPosition().sub(parentPos));
                startLocalX = headLocal.x;
            }
        }
    }
    if (headTransform) {
