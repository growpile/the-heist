...
// @input bool lockPlaneX
// @input bool lockPlaneY
// @input bool lockPlaneZ
...
function clampToPlane(pos) {
    ...
    var lockRef = planeLockLocal || vec3.zero();
    if (script.lockPlaneX) { local.x = lockRef.x; }
    if (script.lockPlaneY) { local.y = lockRef.y; }
    if (script.lockPlaneZ) { local.z = lockRef.z; }
...
function applyConstraints(pos) {
    ...
    return clampToMaxLength(clamped);
}
...
function init() {
    if (baseTransform && headTransform) {
        ...
        if (baseRotInv) {
            planeLockLocal = baseRotInv.multiplyVec3(headTransform.getWorldPosition().sub(basePos));
        }
    }
    socketsList = [];
