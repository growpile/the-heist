...
function updateReturn() {
    ...
    if (startHeadLocal && baseTransform) {
        targetHead = baseTransform.localPointToWorld(startHeadLocal);
    }
    if (startHandleLocal && baseTransform) {
        targetHandle = baseTransform.localPointToWorld(startHandleLocal);
    }
...
function updatePipe() {
    ...
    var points = [];
    var startPos = baseTransform.worldPointToLocal(startPosWorld);
    var endPos = baseTransform.worldPointToLocal(endPosWorld);
...
function init() {
    if (headTransform) {
        ...
        startHeadLocal = baseTransform ? baseTransform.worldPointToLocal(startHeadPos) : headTransform.getLocalPosition();
    }
    if (handleTransform) {
        startHandlePos = handleTransform.getWorldPosition();
        startHandleLocal = baseTransform ? baseTransform.worldPointToLocal(startHandlePos) : handleTransform.getLocalPosition();
    }
    ...
    if (startHeadLocal) {
        var basePos = baseTransform ? baseTransform.getWorldPosition() : vec3.zero();
        var localOffset = baseTransform
            ? baseTransform.worldPointToLocal(basePos.add(returnOffset)).sub(baseTransform.worldPointToLocal(basePos))
            : vec3.zero();
        startHeadLocal = startHeadLocal.add(localOffset);
    }
    if (startHandleLocal) {
        var basePos2 = baseTransform ? baseTransform.getWorldPosition() : vec3.zero();
        var localOffset2 = baseTransform
            ? baseTransform.worldPointToLocal(basePos2.add(returnOffset)).sub(baseTransform.worldPointToLocal(basePos2))
            : vec3.zero();
