...
// @input SceneObject wireCage "Wire Bounds Collider"
...
function clampToCage(pos) {
    if (!script.wireCage) {
        return pos;
    }
    var collider = script.wireCage.getComponent("Physics.ColliderComponent");
    if (!collider || !collider.shape || !collider.shape.size) {
        return pos;
    }
    var size = collider.shape.size;
    var half = new vec3(size.x * 0.5, size.y * 0.5, size.z * 0.5);
    var cageTransform = script.wireCage.getTransform();
    var cagePos = cageTransform.getWorldPosition();
    var cageRot = cageTransform.getWorldRotation();
    var cageRotInv = invertQuat(cageRot);
    if (!cageRotInv) { return pos; }
    var cageScale = cageTransform.getWorldScale();
    var invScale = new vec3(
        cageScale.x !== 0 ? 1 / cageScale.x : 0,
        cageScale.y !== 0 ? 1 / cageScale.y : 0,
        cageScale.z !== 0 ? 1 / cageScale.z : 0
    );

    var localOffset = pos.sub(cagePos);
    localOffset = new vec3(
        localOffset.x * invScale.x,
        localOffset.y * invScale.y,
        localOffset.z * invScale.z
    );
    var localPos = cageRotInv.multiplyVec3(localOffset);
    var clampedLocal = new vec3(
        clamp(localPos.x, -half.x, half.x),
        clamp(localPos.y, -half.y, half.y),
        clamp(localPos.z, -half.z, half.z)
    );
    var scaledLocal = new vec3(
        clampedLocal.x * cageScale.x,
        clampedLocal.y * cageScale.y,
        clampedLocal.z * cageScale.z
    );
    return cageRot.multiplyVec3(scaledLocal).add(cagePos);
}
...
function clampToPlane(pos) {
    ...
    var baseRotInv = invertQuat(baseRot);
    if (!baseRotInv) {
        return pos;
    }
    var local = baseRotInv.multiplyVec3(pos.sub(basePos));
    var zRef = (startLocalZ !== null) ? startLocalZ : local.z;
    local.z = zRef;
    return baseRot.multiplyVec3(local).add(basePos);
}

function applyConstraints(pos) {
    return clampToCage(pos);
}
...
function updateFollow() {
    ...
    if (isFollowing) {
        ...
        handleTransform.setWorldPosition(target);
...
function init() {
    if (baseTransform && headTransform) {
        var basePos = baseTransform.getWorldPosition();
        var baseRotInv = invertQuat(baseTransform.getWorldRotation());
        if (baseRotInv) {
            var headLocal = baseRotInv.multiplyVec3(headTransform.getWorldPosition().sub(basePos));
            startLocalZ = headLocal.z;
        }
    }
    socketsList = [];
