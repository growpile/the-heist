...
function applyConstraints(pos) {
    var clamped = clampToAngleXY(pos);
    return clampToMaxLength(clamped);
}

function clampToAngleXY(pos) {
    if (!baseTransform) {
        return pos;
    }
    var basePos = baseTransform.getWorldPosition();
    ...
    var r = Math.sqrt(local.x * local.x + local.y * local.y);
    if (r < 1e-5) {
        return pos;
    }
    var angle = Math.atan2(local.y, local.x);
    var frontAngle = -Math.PI * 0.5;
    var maxOffset = 40 * (Math.PI / 180);
    var offset = angle - frontAngle;
    while (offset > Math.PI) { offset -= Math.PI * 2; }
    while (offset < -Math.PI) { offset += Math.PI * 2; }
    if (offset < -maxOffset) { offset = -maxOffset; }
    if (offset > maxOffset) { offset = maxOffset; }
    var clampedAngle = frontAngle + offset;
    local.x = Math.cos(clampedAngle) * r;
    local.y = Math.sin(clampedAngle) * r;
...
function updateFollow() {
    ...
    if (script.debugText && baseTransform) {
        ...
        var angleXY = getHeadAngleXYDeg();
        script.debugText.text =
            "d " + distDisplay.toFixed(2) +
            " | XY " + angleXY.toFixed(1);
    }
...
function getHeadAngleXYDeg() {
    if (!headTransform || !baseTransform) {
        return 0;
    }
    var basePos = baseTransform.getWorldPosition();
    var headPos = headTransform.getWorldPosition();
    ...
    var local = baseRotInv.multiplyVec3(headPos.sub(basePos));
    if (local.length < 1e-5) {
        return 0;
    }
    var radToDeg = 57.2957795;
    var angle = Math.atan2(local.y, local.x) * radToDeg;
    angle += 90;
