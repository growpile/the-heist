...
function updateFollow() {
    ...
    anglePrintTimer += getDeltaTime();
    if (anglePrintTimer >= 0.2) {
        anglePrintTimer = 0;
        print("[WireConnector] Head angle (deg): " + getHeadAngleDeg().toFixed(2));
    }
}
...
function getHeadAngleDeg() {
    if (!headTransform || !baseTransform) {
        return 0;
    }
    var basePos = baseTransform.getWorldPosition();
    var headPos = headTransform.getWorldPosition();
    var dir = headPos.sub(basePos);
    if (dir.length < 1e-5) {
        return 0;
    }
    dir = dir.normalize();
    var forward = baseTransform.getWorldRotation().multiplyVec3(vec3.forward()).normalize();
    var dot = clamp(dir.dot(forward), -1, 1);
    return Math.acos(dot) * 57.2957795;
}
