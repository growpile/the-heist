...
function updateFollow() {
    ...
    if (script.debugText && baseTransform) {
        ...
        var angles = getHeadAnglesDeg();
        script.debugText.text =
            "d " + distDisplay.toFixed(2) +
            " | XY " + angles.xy.toFixed(1) +
            " | XZ " + angles.xz.toFixed(1) +
            " | YZ " + angles.yz.toFixed(1);
    }
...
function getHeadAnglesDeg() {
    if (!headTransform || !baseTransform) {
        return { xy: 0, xz: 0, yz: 0 };
    }
    var basePos = baseTransform.getWorldPosition();
    var headPos = headTransform.getWorldPosition();
    ...
    return {
        xy: Math.atan2(local.y, local.x) * radToDeg,
        xz: Math.atan2(local.z, local.x) * radToDeg,
        yz: Math.atan2(local.z, local.y) * radToDeg,
    };
}
