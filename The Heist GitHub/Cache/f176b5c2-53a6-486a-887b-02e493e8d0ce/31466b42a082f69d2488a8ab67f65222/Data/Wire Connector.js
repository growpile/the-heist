...
// @input vec3 cageSize = "{10,10,10}"
// @input SceneObject cageCenter
...
function clampToCage(pos) {
    if (!script.cageCenter) {
        return pos;
    }
    var size = script.cageSize || vec3.zero();
    var half = new vec3(size.x * 0.5, size.y * 0.5, size.z * 0.5);
    var cageTransform = script.cageCenter.getTransform();
