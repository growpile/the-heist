...
function applyConstraints(pos) {
    var clamped = clampToPlane(pos);
    clamped = clampToCage(clamped);
    clamped = clampToPlane(clamped);
    clamped = clampToMaxLength(clamped);
    return clampToPlane(clamped);
}

function projectHandleToPlaneWorld(handleWorld) {
    if (!baseTransform) {
        return handleWorld;
    }
    var basePos = baseTransform.getWorldPosition();
    ...
    var local = baseRotInv.multiplyVec3(handleWorld.sub(basePos));
    var zRef = (startLocalZ !== null) ? startLocalZ : local.z;
    local.z = zRef;
    return baseRot.multiplyVec3(local).add(basePos);
}
...
function updateFollow() {
    ...
    if (isFollowing) {
        var target = projectHandleToPlaneWorld(handleTransform.getWorldPosition());
