...
export class GameFlowController extends BaseScriptComponent {
  ...
  @input
  @allowUndefined
  @hint("Table grid / ground plane shader. Fades in during gameplay and rotates with the safe.")
  groundPlaneMaterial: Material
  ...
  handleSafeComplete(safeType: SafeType, solvedInSeconds: number): void {
    ...
    global.utils.animateMaterialProperty(this.groundPlaneMaterial, "mainPass.opacityMultiplier", 0, 0.25)
    global.utils.animateMaterialProperty(this.groundPlaneMaterial, "mainPass.size", 0, 0.25)
  ...
  private animateGroundPlane(
    property: string,
    value: number,
    duration: number,
    callback?: () => void
  ): void {
    if (!this.groundPlaneMaterial) {
      callback?.()
      return
    }
    global.utils.animateMaterialProperty(this.groundPlaneMaterial, property, value, duration, callback)
  }
  ...
  private syncGroundPlaneMaterialToRotationManager(): void {
    ...
    mgr.setGroundMaterial(this.groundPlaneMaterial)
  }
  ...
  private safeIntro(safeType: SafeType): void {
    ...
    this.syncGroundPlaneMaterialToRotationManager()
    global.utils.animateMaterialProperty(this.groundPlaneMaterial, "mainPass.opacityMultiplier", 1, 0.25)
    global.utils.delay(1, () => {
      global.utils.animateMaterialProperty(this.groundPlaneMaterial, "mainPass.size", 1, 0.25)
