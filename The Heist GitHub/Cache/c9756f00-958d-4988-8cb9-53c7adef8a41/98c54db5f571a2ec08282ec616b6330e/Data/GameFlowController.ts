...
export class GameFlowController extends BaseScriptComponent {
  ...
  @input
  @allowUndefined
  rotationManager: ScriptComponent
  ...
  startSolo(): void {
    ...
    this.menuController?.hide(() => {
      const safeType: SafeType = global.appState.checkStorage("tutorialPlayed") ? "solo" : "tutorial"
  ...
  private beginSurfacePlacement(safeType: SafeType): void {
    ...
    const settings = new PlacementSettings(
    ...
    const controller = SurfacePlacementController.getInstance()
    controller.startSurfacePlacement(settings, (pos: vec3, rot: quat) => {
  ...
  private safeIntro(safeType: SafeType): void {
    ...
    const scriptComponents = safeObject.getComponents("Component.ScriptComponent") as ScriptComponent[]
    let safe: Safe | null = null
    for (const comp of scriptComponents) {
      const candidate = comp as unknown as Safe
    ...
    global.utils.delay(1, () => {
      ...
      global.utils.animateScale(safeObject, true, overshootScale, 0.2, () => {
        global.utils.animateScale(safeObject, true, finalScale, 0.05, () => {
          ...
          if (this.rotationManager && typeof (this.rotationManager as any).setCanRotate === "function") {
            ;(this.rotationManager as any).setCanRotate(true)
          }
          safe.beginSolve()
        })
      })
