...
export class GameFlowController extends BaseScriptComponent {
  ...
  @input
  @allowUndefined
  @hint("Ground Plane scene object — material is read from its Render Mesh Visual at runtime.")
  ...
  playFailExplosionAndScaleAway(callback?: () => void): void {
    ...
    this.shrinkGroundPlaneForFail()
    this.scaleSafeRootAway(() => callback?.())
  }

  /** Fail explosion: shrink grid size, then fade opacity halfway through. */
  private shrinkGroundPlaneForFail(): void {
    const duration = 0.25
    this.animateGroundPlaneProperty("size", 0, duration)
    global.utils.delay(duration * 0.5, () => {
      this.animateGroundPlaneProperty("opacityMultiplier", 0, duration)
    })
  }
  ...
  handleSafeFailed(): void {
    ...
    // Show menu before destroying the prefab — destroy runs from Safe.backToMenu on that prefab.
    this.menuController?.showMainMenu()
    global.utils.delay(0, () => {
      this.activeSafeComponent = null
      if (prefabRoot) {
        prefabRoot.destroy()
      }
      if (global.appState.safe?.object === prefabRoot) {
        global.appState.safe = undefined as any
      }
    })
  }
  ...
  private resolveGroundPlaneMaterial(): Material | null {
    if (!this.groundPlane) {
      return null
    }
  ...
  private getGroundPlanePropertyPath(material: Material, key: string): string | null {
  ...
  private setGroundPlaneProperty(key: string, value: number): void {
    const material = this.resolveGroundPlaneMaterial()
    if (!material) {
      return
    }
    const path = this.getGroundPlanePropertyPath(material, key)
    if (!path) {
      return
    }
  ...
  private animateGroundPlaneProperty(
    key: string,
    value: number,
    duration: number,
    callback?: () => void
    ...
    global.utils.animateMaterialProperty(material, path, value, duration, callback)
  }

  /** Hides the table grid immediately (menu / pre-game). */
  private resetGroundPlaneHidden(): void {
    this.setGroundPlaneProperty("opacityMultiplier", 0)
    this.setGroundPlaneProperty("size", 0)
    this.setGroundPlaneProperty("rotation", 0)
    this.syncGroundPlaneMaterialToRotationManager()
  }

  /** Fades grid in when the safe session starts. */
  private showGroundPlane(): void {
    this.resetGroundPlaneHidden()
    this.animateGroundPlaneProperty("opacityMultiplier", 1, 0.25)
    global.utils.delay(1, () => {
      this.animateGroundPlaneProperty("size", 1, 0.25)
    })
  }

  /** Fades grid out when leaving gameplay. */
  private hideGroundPlane(): void {
    this.animateGroundPlaneProperty("opacityMultiplier", 0, 0.25)
    this.animateGroundPlaneProperty("size", 0, 0.25)
    this.setGroundPlaneProperty("rotation", 0)
  }

  /** Scales safeRoot (or prefab root fallback), then destroys the prefab instance. */
  private scaleSafeAwayAndDestroy(onComplete: () => void): void {
    const safeState = global.appState.safe
  ...
  private syncGroundPlaneMaterialToRotationManager(): void {
    ...
    mgr.setGroundMaterial(this.resolveGroundPlaneMaterial())
  }
