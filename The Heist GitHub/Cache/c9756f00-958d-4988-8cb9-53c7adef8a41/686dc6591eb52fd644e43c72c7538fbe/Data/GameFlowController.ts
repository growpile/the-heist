...
export class GameFlowController extends BaseScriptComponent {
  ...
  private safeIntro(safeType: SafeType): void {
    ...
    const safeComp = safeObject.getComponent("Component.ScriptComponent") as ScriptComponent
    const safe = safeComp as unknown as Safe
    this.activeSafeComponent = safe

    const gameFlowScript = this.getSceneObject().getComponent(this.getTypeName()) as ScriptComponent
    if (safe && (safe as any).gameFlow === undefined) {
      ;(safe as any).gameFlow = gameFlowScript
    }
  ...
  private async beginNetworkFlow(): Promise<void> {
    ...
    if (this.cameraAccessHandler && typeof (this.cameraAccessHandler as any).startStreaming === "function") {
      ;(this.cameraAccessHandler as any).startStreaming({
        externalScript: this.getSceneObject().getComponent(this.getTypeName()),
