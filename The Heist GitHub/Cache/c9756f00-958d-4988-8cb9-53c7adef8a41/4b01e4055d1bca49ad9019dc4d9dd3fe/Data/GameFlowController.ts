...
export class GameFlowController extends BaseScriptComponent {
  ...
  openSettings(): void {
    ...
    this.menuController?.showOverlay("settings", () => {
      this.syncSettingsControlsFromStorage()
    })
  }

  /** Apply persisted volume / gloves to UI after settings overlay is enabled (Slider inits on enable). */
  private syncSettingsControlsFromStorage(): void {
    const volume = global.appState.checkStorage("masterVolume") as number
    const glovesOn = global.appState.checkStorage("enabledGloves") as boolean

    if (this.volumeSlider) {
      const slider = this.volumeSlider as {
        currentValue?: number
        updateCurrentValue?: (value: number, shouldAnimate?: boolean) => void
      }
      if (typeof slider.updateCurrentValue === "function") {
        slider.updateCurrentValue(volume, false)
      } else if (slider.currentValue !== undefined) {
        slider.currentValue = volume
      }
    }
    if (this.glovesToggle) {
      ;(this.glovesToggle as {isOn?: boolean}).isOn = glovesOn
    }
  }
