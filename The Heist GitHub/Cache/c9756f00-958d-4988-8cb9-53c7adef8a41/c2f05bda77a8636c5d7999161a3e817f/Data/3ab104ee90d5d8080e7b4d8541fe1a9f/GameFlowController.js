...
let GameFlowController = (() => {
    ...
    var GameFlowController = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.cameraAccessHandler = this.cameraAccessHandler;
        ...
        startSolo() {
            ...
            this.menuController?.hide(() => {
                const safeType = global.appState.checkStorage("tutorialPlayed") ? "solo" : "tutorial";
                this.beginSurfacePlacement(safeType);
            });
        }
        beginSurfacePlacement(safeType) {
            if (!this.safeOrigin) {
                print("[GameFlowController] No safeOrigin set; falling back to direct safe intro");
                this.safeIntro(safeType);
                return;
            }
            this.surfacePlacementActive = true;
            const settings = new PlacementSettings_1.PlacementSettings(PlacementSettings_1.PlacementMode.NEAR_SURFACE, true, new vec3(10, 10, 0), (pos) => this.onSurfaceSliderUpdated(pos));
            const controller = SurfacePlacementController_1.SurfacePlacementController.getInstance();
            controller.startSurfacePlacement(settings, (pos, rot) => {
                this.onSurfacePlaced(safeType, pos, rot);
            });
        }
        ...
        handleSafeComplete(safeType, solvedInSeconds) {
            print(safeType);
            this.gameUI?.setSolvedSeconds(solvedInSeconds);
        ...
        backToMenu() {
            ...
            if (state === "winPostGame" || state === "losePostGame") {
                if (state === "winPostGame") {
                    this.gameUI?.hideSolved(() => {
                        this.menuController?.show();
                    });
                }
                else {
                    this.gameUI?.hideTimedOut(() => {
                        this.menuController?.show();
                    });
                }
            }
            else if (state === "tutorialWinPostGame") {
                this.gameUI?.hideTutorialSolved(() => {
                    this.menuController?.show();
                });
            }
        }
        ...
        exitSettings(callback) {
            ...
            this.gameUI?.hideSettings(() => {
                this.menuController?.show(callback);
            });
        }
        rescanSurface() {
            if (global.appState.inTransition) {
        ...
        safeIntro(safeType) {
            ...
            const scriptComponents = safeObject.getComponents("Component.ScriptComponent");
            let safe = null;
            for (const comp of scriptComponents) {
                const candidate = comp;
            ...
            global.utils.delay(1, () => {
                ...
                global.utils.animateScale(safeObject, true, overshootScale, 0.2, () => {
                    global.utils.animateScale(safeObject, true, finalScale, 0.05, () => {
                        ...
                        if (this.rotationManager && typeof this.rotationManager.setCanRotate === "function") {
                            ;
                            this.rotationManager.setCanRotate(true);
                        }
                        safe.beginSolve();
                    });
                });
        ...
        async beginNetworkFlow() {
            ...
            if (this.cameraAccessHandler && typeof this.cameraAccessHandler.startStreaming === "function") {
                ;
                this.cameraAccessHandler.startStreaming({
                    externalScript: this,
            ...
            this.gameUI?.hideLoading(() => {
                this.gameUI?.showRoom();
            });
        }
        networkFlowFailed() {
        ...
        stopNetworkStreaming() {
            global.utils.delay(3, () => {
                this.sendToRealtime = false;
                if (this.cameraAccessHandler && typeof this.cameraAccessHandler.stopStreaming === "function") {
                    ;
                    this.cameraAccessHandler.stopStreaming();
                }
        ...
        introLogoSequence() {
            if (this.introSkipped || !this.tweens) {
                return;
            }
        ...
        playareaSetup() {
            if (this.anchorManager) {
                this.anchorManager.enabled = true;
            }
        }
    };
