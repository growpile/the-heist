...
function setupButtonColors() {
    var materials = [
        { material: redMaterial, color: colorNames.RED },
        { material: greenMaterial, color: colorNames.GREEN },
        { material: blueMaterial, color: colorNames.BLUE },
        { material: yellowMaterial, color: colorNames.YELLOW }
    ...
    for (let i = 0; i < buttonComponents.length; i++) {
        ...
        var assignedEntry = materials[materialIndex];
        var clonedMaterial = assignedEntry.material.clone();
        visual.mainMaterial = clonedMaterial;
        buttonColors[i] = assignedEntry.color;
    }
}
