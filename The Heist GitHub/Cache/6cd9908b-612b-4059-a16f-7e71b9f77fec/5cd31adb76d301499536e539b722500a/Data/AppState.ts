import "./HeistGlobals"
import type {HeistAppState} from "./HeistGlobals"
 ...
 * Replace the legacy AppState.js script on the Prerequisites → AppState object.
 */
@component
export class AppState extends BaseScriptComponent {
  onAwake(): void {
    this.initGlobalAppState()
  }

  private initGlobalAppState(): void {
    const appState: HeistAppState = {
      currentState: "intro",
      anchorManager: null,
      inTransition: false,
      ...
      safe: {},
      storage: {},
      checkStorage: (key: string) => this.checkStorage(key),
      setStorage: (key: string, value: boolean | number | string) => this.setStorage(key, value)
    }

    global.appState = appState
  }

  private checkStorage(key: string): boolean | number | string {
  ...
  private setStorage(key: string, value: boolean | number | string): void {
    const def = STORAGE_DEFAULTS[key]
    if (!def) {
      return
    }
...
// Re-export types used by this module for other scripts
import type {HeistAppState} from "./HeistGlobals"
