/** Persistent storage + session state on global.appState. */
...
function storeHasKey(store: any, key: string, type?: StorageType): boolean {
  if (!store) {
    return false
  }
...
function initGlobalAppState(): void {
  ...
  global.appState = {
    ...
    getMasterVolume: () => checkStorage("masterVolume") as number,
