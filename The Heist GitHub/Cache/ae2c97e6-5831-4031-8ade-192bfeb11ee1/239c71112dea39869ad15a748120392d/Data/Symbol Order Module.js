...
function applySymbols(layout, textureMap) {
    ...
    for (var i = 0; i < symbolImageComponents.length; i++) {
        var symbolId = layout[i];
        var texture = textureMap[symbolId];
        var newMaterial = symbolImageComponents[i].mainMaterial.clone();
        symbolImageComponents[i].clearMaterials();
        symbolImageComponents[i].addMaterial(newMaterial);
        symbolImageComponents[i].mainMaterial.mainPass.symbolMap = texture;
    }
}
