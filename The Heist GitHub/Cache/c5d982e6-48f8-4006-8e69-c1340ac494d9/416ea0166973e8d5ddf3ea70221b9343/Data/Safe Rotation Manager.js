...
function isHitArea(hitInfo, areaCollider) {
    if (!hitInfo || !hitInfo.hit || !hitInfo.hit.collider || !areaCollider) {
        return false;
    }
    if (hitInfo.hit.collider === areaCollider) {
        return true;
    }
    var areaObj = areaCollider.getSceneObject ? areaCollider.getSceneObject() : null;
...
function checkEditorClick() {
    ...
    if (primaryInteractor.previousTrigger !== InteractorTriggerType.None &&
        ...
        if (isHitArea(primaryInteractor.targetHitInfo, leftArea)) {
            leftRotation("Editor");
        } else if (isHitArea(primaryInteractor.targetHitInfo, rightArea)) {
