...
/** @type {Material} */
var groundMaterial = script.groundMaterial;
...
function animateYawDelta(deltaRadians, duration) {
    ...
    global.playSfx(global.utils.rng(22, 25), 1, global.appState.checkStorage("masterVolume") * 0.8);
    if (!baseEuler) {
    ...
    if (groundAnim && groundAnim.updateEvent) {
        groundAnim.updateEvent.enabled = false;
        groundAnim.updateEvent = null;
    }
    var groundStart = 0;
    var groundTarget = deltaRadians >= 0 ? 1 : -1;
    if (groundMaterial && groundMaterial.mainPass && groundMaterial.mainPass.rotation !== undefined) {
        groundStart = groundMaterial.mainPass.rotation;
        groundAnim = {
            startTime: getTime(),
            updateEvent: script.createEvent("UpdateEvent")
        };
        groundAnim.updateEvent.bind(function() {
            var elapsed = getTime() - groundAnim.startTime;
            var t = Math.min(elapsed / duration, 1);
            var smoothT = t * t * (3 - 2 * t);
            groundMaterial.mainPass.rotation = groundStart + (groundTarget - groundStart) * smoothT;
            if (t >= 1) {
                groundMaterial.mainPass.rotation = 0;
                groundAnim.updateEvent.enabled = false;
                groundAnim.updateEvent = null;
            }
        });
...
function isIconReady(index) {
    var material = getIconMaterial(index);
    var value = getIconProgress(material);
    if (value === null || value === undefined) { return true; }
    return value <= 0.001;
}

function animateIconProgress(index, targetValue, duration, callback) {
    var material = getIconMaterial(index);
    if (!material) { return; }
...
function getIconOpacity(material) {
    if (!material) { return null; }
...
function setIconOpacity(material, value) {
    if (!material) { return; }
    if (material.mainPass && material.mainPass.opacityMultiplier !== undefined) {
...
function animateIconOpacity(index, targetValue, duration) {
    var material = getIconMaterial(index);
    if (!material) { return; }
    var startValue = getIconOpacity(material);
    if (startValue === null || startValue === undefined) { return; }

    if (iconOpacityAnims[index] && iconOpacityAnims[index].updateEvent) {
        iconOpacityAnims[index].updateEvent.enabled = false;
        iconOpacityAnims[index].updateEvent = null;
    }
    ...
    iconOpacityAnims[index] = animData;
    ...
    animData.updateEvent.bind(function() {
        ...
        if (t >= 1) {
            setIconOpacity(material, targetValue);
            animData.updateEvent.enabled = false;
            animData.updateEvent = null;
        }
    });
...
function updateHoldState(index, state, isActive, handLabel, triggerFn) {
    ...
    if (isActive) {
        if (!state.active) {
            ...
            var triggerTime = HOLD_DURATION * 0.7;
            if (state.__triggerDelayId) {
                global.utils.invalidateDelay(state.__triggerDelayId);
            }
            state.__triggerDelayId = "rotateHold_" + index;
            global.utils.delay(state.__triggerDelayId, triggerTime, function() {
...
function leftRotation(handLabel) {
    ...
    global.tweenManager.startTween(rotateIcons[0].getSceneObject(), "rotate-icon-animation");
...
function rightRotation(handLabel) {
    if (!script.canRotate) { return; }
    if (rotationAnim && rotationAnim.updateEvent) { return; }
    global.tweenManager.startTween(rotateIcons[1].getSceneObject(), "rotate-icon-animation");
...
script.setCanRotate = function(state) {
    ...
    var targetOpacity = script.canRotate ? 1 : 0;
    animateIconOpacity(0, targetOpacity, 0.25);
    animateIconOpacity(1, targetOpacity, 0.25);
};
...
function checkEditorClick() {
    ...
    if (primaryInteractor.previousTrigger !== InteractorTriggerType.None &&
        ...
        if (lastEditorStartArea === LEFT_AREA_NAME && isHitAreaName(hitInfo, LEFT_AREA_NAME) && isIconReady(0)) {
            animateIconProgress(0, 1, 0.25);
            global.utils.delay(0.25, function() {
                animateIconProgress(0, 0, RELEASE_DURATION);
            });
            leftRotation("Editor");
        } else if (lastEditorStartArea === RIGHT_AREA_NAME && isHitAreaName(hitInfo, RIGHT_AREA_NAME) && isIconReady(1)) {
