...
script.getRemainingSeconds = function() {
    if (!countdownActive) {
        return Math.max(0, countdownSeconds);
    }
    var remaining = countdownSeconds - countdownAccumulator;
...
script.beginSolve = function() {
    ...
    if (script.safeType == "tutorial") {
        script.bombTimer = 999
    }
    solveStarted = true;
    startNormalTicking();
    global.utils.delay(0.5, function() {
        startCountdown(script.bombTimer);
        if(script.safeType == "tutorial") global.leftRotateHint();
    });
};
...
function spawnModule(moduleId, slotId) {
    if (!moduleId || slotId === undefined || slotId === null) {
        return null;
    }
    var moduleConfig = null;
    for (var i = 0; i < modules.length; i++) {
        if (modules[i] && modules[i].moduleId === moduleId) {
            moduleConfig = modules[i];
            break;
        }
    }
    if (!moduleConfig || !moduleConfig.prefab || !moduleSlots[slotId]) {
        return null;
    }
    var moduleObject = moduleConfig.prefab.instantiate(moduleSlots[slotId]);
    return { moduleId: moduleConfig.moduleId, moduleObject: moduleObject };
}
...
function checkSafeSolved() {
    ...
    timerProgressActive = false;
    stopTickingSfx();
    var solvedInSeconds = 0;
    if (solveStarted) {
        solvedInSeconds = Math.max(0, (script.bombTimer - countdownSeconds) + countdownAccumulator);
    }
    ...
    global.utils.delay(0.5, function() {
        ...
        global.utils.delay(0.5, function() {
            ...
            global.playSfx(26, 1, global.appState.checkStorage("masterVolume") * 0.8);
            global.playSfx(27, 1, global.appState.checkStorage("masterVolume") * 0.8);
            global.tweenManager.startTween(script.getSceneObject(), "safe-door-open", function() {
                global.utils.delay(3, function() {
                    global.safeComplete(script.safeType, solvedInSeconds);
                });
            });
        });
    });
...
script.init = function(safeType) {
    ...
    if(safeType != "tutorial") {
        ...
        // tutorial
        var frontSlotSpawnResult = spawnModule("wireFusebox", 0);
        moduleList.push("wireFusebox");
        moduleObjects.push(frontSlotSpawnResult.moduleObject);

        var leftSlotModuleSpawnResult = spawnModule("colorOrder", 1);
        moduleList.push("colorOrder");
        moduleObjects.push(leftSlotModuleSpawnResult.moduleObject);

        var rightSlotModuleSpawnResult = spawnModule("symbolOrder", 2);
        moduleList.push("symbolOrder");
        moduleObjects.push(rightSlotModuleSpawnResult.moduleObject);
    }
    ...
    if(safeType == "tutorial") {
        moduleObjects[0].getComponent("Component.ScriptComponent").tutorialInstaComplete();
        moduleObjects[2].getComponent("Component.ScriptComponent").tutorialInstaComplete();
    }
