...
function cloneModuleDisplayMaterials() {
    for(var m = 0; m < moduleSlots; m++) {
        script.moduleSlots[slotId].getChild(0).getChild(0).getChild(0).getComponent('Component.Image')
        script.getSceneObject()
    }
}

script.completeModule = function(slotId) {
    var textComponent = script.moduleSlots[slotId].getChild(0).getChild(0).getComponent("Component.Text");
