...
function clampHeadToCage() {
    ...
    var center = script.wireCage.getTransform().getWorldPosition();
    var pos = headTransform.getWorldPosition();

    var clamped = new vec3(
        clamp(pos.x, center.x - half.x, center.x + half.x),
        clamp(pos.y, center.y - half.y, center.y + half.y),
        clamp(pos.z, center.z - half.z, center.z + half.z)
    );

    if (!pos.equal(clamped)) {
        headTransform.setWorldPosition(clamped);
    }
}
