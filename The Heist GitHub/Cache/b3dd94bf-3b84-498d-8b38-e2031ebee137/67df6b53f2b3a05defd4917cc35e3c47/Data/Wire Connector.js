...
var occupancyListKey = "wireSocketOccupancy";
...
function ensureOccupancyList() {
    if (!global[occupancyListKey]) {
        global[occupancyListKey] = [];
    }
}

function isSocketOccupied(socket) {
    ensureOccupancyList();
    var list = global[occupancyListKey];
...
function occupySocket(socket) {
    ensureOccupancyList();
    releaseSocket();
    global[occupancyListKey].push({ socket: socket, wire: script });
...
function trySnapToAvailableSocket() {
    if (!socketsList || socketsList.length === 0) {
        return false;
    }
    var headPos = headTransform.getWorldPosition();
    var best = null;
