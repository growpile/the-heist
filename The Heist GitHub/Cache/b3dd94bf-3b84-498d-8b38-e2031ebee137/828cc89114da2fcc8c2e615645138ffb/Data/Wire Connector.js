...
function updatePipe() {
    ...
    for (var i = 0; i <= segments; i++) {
        var t = segments === 0 ? 0 : i / segments;
        var curvedT = curveProgress(t);
        var p = vec3.lerp(startPos, endPos, curvedT);
        points.push(p);
    }
...
function curveProgress(t) {
    t = clamp(t, 0, 1);
    var delta = t - 0.5;
    var curved = 0.5 + 0.5 * Math.sign(delta) * Math.pow(Math.abs(delta) * 2, 1.5);
