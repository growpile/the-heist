...
function worldToPipeLocal(worldPos) {
    // Convert world position into the local space of the parent (script's scene object)
    if (!planeTransform) { return worldPos; }
    var pipePos = planeTransform.getWorldPosition();
    var pipeRot = planeTransform.getWorldRotation();
...
function init() {
    ...
    pipeObj.setParent(script.getSceneObject());
