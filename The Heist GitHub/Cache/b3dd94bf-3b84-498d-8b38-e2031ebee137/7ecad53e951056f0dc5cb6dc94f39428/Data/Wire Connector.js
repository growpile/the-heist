...
function clampHeadToCage() {
    if (!script.wireCage || !headTransform) {
        return;
    }
    ...
    if (!cageRotInv) { return; }

    var pos = headTransform.getWorldPosition();
    // Convert to cage local space
    ...
    if (!localPos.equal(clampedLocal)) {
        var clampedWorld = cageRot.multiplyVec3(clampedLocal).add(cagePos);
        headTransform.setWorldPosition(clampedWorld);
    }
}
