...
function worldToPipeLocal(worldPos) {
    ...
    var rotated = invRot ? invRot.multiplyVec3(offset) : offset;
    var ws = pipeTransform.getWorldScale ? pipeTransform.getWorldScale() : vec3.one();
    pipeScaleFactor = Math.max(0.0001, (Math.abs(ws.x) + Math.abs(ws.y) + Math.abs(ws.z)) / 3);
    return new vec3(rotated.x / ws.x, rotated.y / ws.y, rotated.z / ws.z);
}
...
function updatePipe() {
    ...
    var radius = (script.pipeRadius || 0.5) / pipeScaleFactor;
