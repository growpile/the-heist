...
function updateFollow() {
    ...
    if (originTransform) {
        ...
        var planePoint = originPos.add(originUp.uniformScale(-3)); // plane 3 units below along local Y
...
function updateReturn() {
    ...
    if (originTransform) {
        var originPos = originTransform.getWorldPosition();
        var originUp = originTransform.getWorldRotation().multiplyVec3(vec3.up()).normalize();
        targetPos = originPos.add(originUp.uniformScale(-2)); // 1 unit below origin along local Y
    } else {
