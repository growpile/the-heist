...
import {
  animateModuleScreenSolved,
  initializeModuleScreenMaterial
} from "./MaterialProgressAnimator"
...
export class SafeModuleManager {
  ...
  cloneModuleDisplayMaterials(): void {
    for (let m = 0; m < this.moduleSlots.length; m++) {
      ...
      initializeModuleScreenMaterial(newMaterial)
    }
  }
  ...
  completeModule(slotId: number): boolean {
    ...
    if (image && image.mainMaterial) {
      animateModuleScreenSolved(image.mainMaterial, 0.25)
    }
