...
/** Match win: brief pause, reset yaw, then next beat before presentation. */
const FAIL_ROTATION_LEAD_SEC = 0.5
const FAIL_AFTER_ROTATION_SEC = 0.5
...
export class SafeSolveSequence {
  ...
  playFailSequence(onReadyForExplosion: () => void): void {
    if (this.failedTriggered) {
      return
    }
    this.failedTriggered = true

    global.utils.delay(FAIL_ROTATION_LEAD_SEC, () => {
      global.resetRotation()
      global.utils.delay(FAIL_AFTER_ROTATION_SEC, () => {
        onReadyForExplosion()
      })
    })
  }
