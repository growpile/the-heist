...
function localToWorldPoint(transform: Transform, localPoint: vec3): vec3 {
  if (!transform) {
    return copyVec3(localPoint)
  }
  return copyVec3(transform.getWorldTransform().multiplyPoint(localPoint))
}
...
function worldToLocalPoint(transform: Transform, worldPoint: vec3): vec3 {
  ...
  return copyVec3(transform.getInvertedWorldTransform().multiplyPoint(worldPoint))
}
...
type TubeRingFrame = {right: vec3; up: vec3}

/** Keep ring orientation consistent along the path (avoids end-cap twist). */
function computeTubeRingFrame(
  tangent: vec3,
  prevRight: vec3 | null,
  prevUp: vec3 | null,
  referenceUp: vec3
): TubeRingFrame {
  const forward = copyVec3(tangent).normalize()

  if (!prevRight || !prevUp) {
    let ref = copyVec3(referenceUp)
    if (Math.abs(forward.dot(ref)) > 0.99) {
      ref = vec3.right()
    }
    const right = forward.cross(ref).normalize()
    const up = right.cross(forward).normalize()
    return {right, up}
  }

  let up = subVec3(prevUp, forward.uniformScale(prevUp.dot(forward)))
  if (up.length < 1e-5) {
    up = subVec3(prevRight, forward.uniformScale(prevRight.dot(forward)))
  }
  up = up.normalize()
  const right = forward.cross(up).normalize()
  const orthoUp = right.cross(forward).normalize()
...
export class WireConnector extends BaseScriptComponent {
  ...
  /** Rest pose in inner reel space — recomputed to world each return so safe rotation stays correct. */
  ...
  /** wireEnd local offset on wireHead — used to derive tip world pos from head each frame. */
  private wireEndOffsetInHeadLocal: vec3 | null = null
  ...
  private refreshTransformRefs(): void {
    ...
    this.refreshWireEndOffsetFromHead()
  }

  /** Cache wireEnd in head-local space so mesh tip tracks head rotation/scale every frame. */
  private refreshWireEndOffsetFromHead(): void {
    if (this.headTransform && this.endTransform) {
      this.wireEndOffsetInHeadLocal = copyVec3(this.endTransform.getLocalPosition())
    } else {
      this.wireEndOffsetInHeadLocal = null
    }
  }
  ...
  /** Legacy Wire Connector.js: inner reel local up → world, scaled −1.25. */
  private getReturnOffset(): vec3 {
    ...
    return copyVec3(
      this.baseTransform
        .getWorldRotation()
        .multiplyVec3(vec3.up())
        .uniformScale(-RETURN_OFFSET_DISTANCE)
    )
  }

  private getReturnOffsetInBaseLocal(): vec3 {
    if (!this.baseTransform) {
      return vec3.zero()
    }
    const returnOffset = this.getReturnOffset()
    const baseWorld = copyVec3(this.baseTransform.getWorldPosition())
    return subVec3(
      worldToLocalPoint(this.baseTransform, addVec3(baseWorld, returnOffset)),
      worldToLocalPoint(this.baseTransform, baseWorld)
    )
  }

  private applyReturnRestOffset(): void {
    const returnOffset = this.getReturnOffset()
    if (returnOffset.length < 1e-5) {
      return
    }
    ...
    if (this.headTransform) {
      ...
      this.startHeadLocal = this.baseTransform
        ...
        : copyVec3(this.headTransform.getLocalPosition())
    }
    if (this.handleTransform) {
      ...
      this.startHandleLocal = this.baseTransform
        ...
        : copyVec3(this.handleTransform.getLocalPosition())
    }

    if (this.headTransform) {
      const restHead = addVec3(copyVec3(this.headTransform.getWorldPosition()), returnOffset)
      this.headTransform.setWorldPosition(restHead)
      this.startHeadPos = copyVec3(restHead)
    }
    if (this.handleTransform) {
      const restHandle = addVec3(copyVec3(this.handleTransform.getWorldPosition()), returnOffset)
      this.handleTransform.setWorldPosition(restHandle)
      this.startHandlePos = copyVec3(restHandle)
    }

    const localOffset = this.getReturnOffsetInBaseLocal()
    if (this.startHeadLocal) {
      this.startHeadLocal = addVec3(this.startHeadLocal, localOffset)
    }
    if (this.startHandleLocal) {
      this.startHandleLocal = addVec3(this.startHandleLocal, localOffset)
    }
  }
  ...
  private getWireStartWorld(): vec3 {
    ...
    for (let i = 0; i < baseObj.getChildrenCount(); i++) {
      ...
      if (child && child.name === "Wire") {
        return localToWorldPoint(child.getTransform(), vec3.zero())
      }
    }
    return localToWorldPoint(this.baseTransform, vec3.zero())
  }

  /** Tip world position from head transform + wireEnd child offset (full hierarchy matrix). */
  private getWireEndWorld(): vec3 {
    if (this.headTransform && this.wireEndOffsetInHeadLocal) {
      return localToWorldPoint(this.headTransform, this.wireEndOffsetInHeadLocal)
    }
    if (this.endTransform) {
      return localToWorldPoint(this.endTransform, vec3.zero())
    }
    if (this.headTransform) {
      return localToWorldPoint(this.headTransform, vec3.zero())
    }
  ...
  private raiseOffsetInBaseLocal(weight: number): vec3 {
    const raiseDir = this.raiseDirection || vec3.zero()
    const raiseDirLen = raiseDir.length
    if (raiseDirLen < 1e-5) {
      return vec3.zero()
    }
    return raiseDir.uniformScale(((this.raiseOffset || 0) * weight) / raiseDirLen)
  }
      ...
      if (raiseKeys.length > 0) {
        ...
        if (raiseDirLen > 1e-5) {
          for (const key of raiseKeys) {
            ...
            points[idx] = points[idx].add(this.raiseOffsetInBaseLocal(weight))
          }
        }
      }
    }
  ...
  private getTubeReferenceUp(): vec3 {
    const raiseDir = this.raiseDirection || vec3.zero()
    if (raiseDir.length > 1e-5) {
      return copyVec3(raiseDir).normalize()
    }
    return vec3.up()
  }
  ...
  private buildTubeMesh(
    ...
    const referenceUp = this.getTubeReferenceUp()
    ...
    for (let i = 0; i < totalRings; i++) {
      ...
      let tangent: vec3
      if (i === 0) {
        tangent = subVec3(path[i + 1], center)
      } else if (i === totalRings - 1) {
        tangent = subVec3(center, path[i - 1])
      } else {
        tangent = subVec3(path[i + 1], path[i - 1])
      }
      tangent = tangent.normalize()

      const frame = computeTubeRingFrame(tangent, prevRight, prevUp, referenceUp)
  ...
  private registerWire(startPos: vec3, endPos: vec3, segments: number): void {
    ...
    for (const entry of list) {
      if (entry.wire === wire) {
        entry.start = copyVec3(startPos)
        entry.end = copyVec3(endPos)
