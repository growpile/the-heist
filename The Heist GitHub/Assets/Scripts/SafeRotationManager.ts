...
export class SafeRotationManager extends BaseScriptComponent {
  ...
  private iconBumpTweens: (StoppableTween | null)[] = []
  ...
  /** Enable or disable rotation zones and fade hold icons. */
  ...
  /** Animate accumulated yaw back to zero (called via global.resetRotation). */
  resetRotation(): void {
  ...
  private getGroundRotation(): number | null {
    return getMaterialScalar(this.groundMaterial, "rotation")
  }

  private setGroundRotation(value: number): void {
    setMaterialScalar(this.groundMaterial, "rotation", value)
  }
  ...
  private playIconBump(index: number): void {
    ...
    const downTween = LSTween.scaleFromToLocal(transform, bump, rest, 120).easing(Easing.Quadratic.In)
    const upTween = LSTween.scaleFromToLocal(transform, rest, bump, 80)
